...
- it inspects a controller-verified worktree/path, branch/ref, optional or inferred base ref, changed files, diff summary, and intake readiness;
...
- `/autoresearch measure [current|<worktree>]` prepares a reviewable `autoresearch_runtime_run({ ...candidate metadata... })` candidate measurement call only when candidate intake readiness is `ready`, otherwise it falls back to candidate-bind intake review;
...
- `$$ autoresearch bind|measure [current|<worktree>]` / `$$ ar bind|measure [current|<worktree>]` open a picker/fallback for candidate-bind intake planning or readiness-gated candidate measurement-call preparation;
...
The twelfth long-campaign dogfood slice is now also landed:
