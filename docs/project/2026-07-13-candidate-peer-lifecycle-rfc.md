...
- synthetic tests prove alias grouping, lock/CAS exclusion, missing and owner-root reconciliation, post-review drift rejection, restoration of staged/unstaged/untracked bytes, immutable integration-proof binding, separate authorization, exact worktree/branch removal, and retained verified archives.
...
Production execution subsequently reached an explicit disposition for every migrated resource: **129 resources / 201 aliases** are `cleaned`, **69 resources / 74 aliases** are `reconciled_missing`, and **28 resources / 34 aliases** are named-owner `deferred`. All 22 originally accepted resources received immutable target-OID integration proof before cleanup. Exact measured pre-cleanup footprints record **130,555,973,628 bytes (121.59 GiB) reclaimed**; restoration archives and deferred resources remain retained. Cohort summaries, authorizations, terminal receipts, and effect receipts remain under `~/.local/state/pi-quests/candidate-lifecycle-v2/`; the checksummed 226-resource closeout aggregate is under `cohorts/final-2026-07-13/`.
