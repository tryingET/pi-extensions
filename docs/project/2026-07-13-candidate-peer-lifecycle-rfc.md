...
3. **Squash/content coverage:** for a path-only acceptance, every declared path resolves to the same exact blob identity and mode at the reviewed candidate HEAD, the named target integration commit, and the immutable target OID. Missing paths, reordered content, target drift, and omitted content from earlier candidate commits fail closed.
...
- synthetic tests prove alias grouping, lock/CAS exclusion, missing reconciliation, post-review drift rejection, restoration of staged/unstaged/untracked bytes, patch-equivalence binding, exact selected-path tree coverage (including reordered-line and omitted-earlier-commit rejection), separate authorization, exact worktree/branch removal, and retained verified archives. Production cohorts additionally exercised durable owner-root reconciliation and content-coverage proofs against immutable owner target OIDs.
...
Production execution subsequently reached an explicit disposition for every migrated resource: **157 resources / 235 aliases** are `cleaned`, **69 resources / 74 aliases** are `reconciled_missing`, and **0 resources / 0 aliases** remain `deferred`. All 22 originally accepted resources received immutable target-OID integration proof before cleanup. The controller-captured footprint ledger records **134,099,998,651 bytes reclaimed** and **0 deferred bytes**; source captures, owner-cohort aggregates, provenance, and checksums are retained because terminal resource records do not themselves store footprint bytes. Restoration archives remain retained and owner-only. Cohort summaries, authorizations, terminal receipts, and effect receipts remain under `~/.local/state/pi-quests/candidate-lifecycle-v2/`; the latest checksummed 226-resource totals are under `cohorts/final-2026-07-17/`.

AK-4001 then re-reviewed the six historical v1 archives associated with missing-resource receipts without executing them. Isolated dogfood restored exact committed heads and trees from five DSPx bundles, but AK-3556 through AK-3560 were already completed through separate owner-accepted main commits, so those archives remain evidence-only and semantically superseded. The sixth archive records an untracked cleanup-dogfood `candidate.txt` path whose bytes were omitted and remain confirmed lost. The other 63 missing resources remain bound to their original receipts. Checksummed review evidence is under `cohorts/v1-archive-review-2026-07-17/`; no v1 archive was upgraded to lifecycle-v2 verified status, no missing receipt was rewritten, and v1 cleanup remains permanently disabled.
...
Candidate creation is fail-closed behind an owner-only lifecycle-v2 admission membrane:

1. Owner configuration names global and exact-repository hard limits for unresolved resources, measured bytes, unresolved age, and active admissions; optional warning thresholds require an exact owner acknowledgement but cannot cross hard limits.
2. Authorization captures the exact repository, objective digest, actor, AK task reference, reservation bytes, expiry, configuration digest, stable resource inventory digest, and active-reservation set.
3. Spawn reacquires the admission lock, recomputes inventory and active reservations, rejects any drift, consumes exactly one matching permit, and records the reservation before Git worktree mutation.
4. Worktree preparation failure receives an explicit admission failure receipt. A created worktree binds the reservation to its peer run, path, and branch. Capacity remains reserved until a terminal lifecycle receipt releases it.
5. Canary mode permits exactly one owner-authorized spawn while the historical hold remains active. It cannot coexist with another active admission.
