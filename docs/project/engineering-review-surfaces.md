...
Refreshed on 2026-05-21 from `node ./scripts/engineering-review-surfaces.mjs --json` after adding `packages/pi-context-packer`.

Snapshot summary:

- package entries audited: `27`
- legacy-full: `18`
- reduced-form: `1`
- policy-only: `0`
- no local surface: `8`
...
- the current eighteen `legacy-full` package-local surfaces still carry both `docs/engineering.local.md` and `policy/engineering-lane.json`:
  - `packages/pi-autoresearch`
  - `packages/pi-better-openai`
  - `packages/pi-context-overlay`
  - `packages/pi-context-packer`
...
- **Cluster A2 — newly present full surfaces needing classification:** `packages/pi-better-openai`, `packages/pi-context-packer`, `packages/pi-designmd-foundry`, `packages/pi-evalset-lab`, `packages/pi-provenance`, and `packages/pi-toolbox-discovery` currently appear in the `legacy-full` bucket, but their package-specific status should be reviewed before they are folded into the boilerplate-only reduction queue.
