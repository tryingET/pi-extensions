...
system4d:
  ...
  fog: "All eight must-fix verification points are folded into the revised RFC and confirmed against source. Two non-material observations were raised at review and resolved post-review (Phase-0 spec line-number drift corrected in the spec; RFC borrow-map L5 row corrected in the RFC) — neither blocked ADR contracts, which are mechanism-level and verified."
---
...
- **O1 — Phase-0 spec line-number drift in the pic evidence index (non-material; corrected post-review).** Re-verified with `rg -n` against the current checkout: two cites had genuinely drifted and are corrected in the Phase-0 spec refresh — `pic/src/page/app.js:2304-2307` for `__picCapabilities` (actual `:4062`; the `:2304` region is unrelated DOM code), and `pic/src/typescript/functions.ts:80-89`/`:80-112` for the tombstone / refuse-removal policy (actual refuse-removal check `:129` (`directDependencies(...).includes(name)`) → refusal `:135` → tombstone append `{deleted:true,name}` `:138` → `reconstruct` `:201`). Two cites are **not** drifts: the oh-my-pi eval chokepoint `runtime.ts:236→237` is **correct** (`wrapCode` at `:236`, `indirectEval` at `:237`; an earlier draft of this note miscounted it as `:237→:238` — withdrawn), and the `pi-evidence-review` schema-id `validation.ts:17`→`:18` off-by-one lives in the first *review memo* (an immutable historical artifact), not the Phase-0 spec, and is left as-is there. **In every case the mechanism is verified real and accurately characterized.** This remains a documentation-precision observation on the *evidence index*, not on the revised RFC or on any ADR contract.
...
Neither O1 nor O2 is a material must-fix for the RFC: the RFC commits **mechanism-level** contracts (two-gate L3; graph = data model + pic resolver; Node reimplementation of the Bun IPC loop; py+js scope; L5 evaluator = TS reimplementation), and every mechanism is verified first-hand. Both were resolved after the re-review as a low-priority housekeeping step (not an ADR blocker): the Phase-0 spec line drifts in O1 are corrected in the spec, and the RFC borrow-map L5 row in O2 is corrected in the RFC.
