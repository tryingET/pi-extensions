...
| Fresh-session handoff generation and launch | `pi-session-compaction` + `pi-little-helpers` | Compaction owns conversation-grounded handoff prompt shape; little-helpers exposes `/fresh-handoff [optional goal]` and the model-callable `fresh_handoff_spawn` tool over one implementation that transports exactly one generated prompt into a fresh clean Ghostty Pi session, auto-submitted as its initial user message. Git/AK/source owners remain runtime authority; launch is not completion proof. |
