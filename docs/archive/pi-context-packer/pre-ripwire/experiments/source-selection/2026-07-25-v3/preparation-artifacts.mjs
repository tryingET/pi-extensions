...
export async function createTraceBundle(traceRecords, artifacts) {
  if (traceRecords.length !== 50) fail("trace bundle requires exactly 50 case records");
  ...
  const gzipResult = await capture(gzipArgv[0], gzipArgv.slice(1), {
    maxBytes: 256 * 1024 * 1024,
  });
  if (gzipResult.code !== 0 || gzipResult.signal !== null || gzipResult.stderr.length !== 0) {
    fail("deterministic trace gzip failed or wrote stderr");
  }
...
export async function createChecksumManifest(allowedPaths, generatedBytes = new Map()) {
...
export function preparationReceipt({ gzipSha256, inputSha256, traceSha256 }) {
  return stableJson({
    status: "prepared-awaiting-independent-pre-run-review",
    repositories: 5,
    cases: 50,
    sciReceiptsAvailableAndComplete: 50,
    ...
    preReviewManifestEntries: PRE_REVIEW_ALLOWED_PATHS.length,
