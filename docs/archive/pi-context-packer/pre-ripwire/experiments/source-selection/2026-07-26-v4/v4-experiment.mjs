...
export const V4_PROTOCOL = "pi-context-packer-source-selection-positive-quality/v4";
...
function aggregate(cases, repositories) {
  ...
  const pooledTotals = Object.fromEntries(ARMS.map((arm) => [arm, summarize(eligibleCases, arm)]));
  const gates = {
...
export function validateV4Experiment(input) {
  exactKeys(input, ["protocol", "repositories", "cases", "provenanceReuse"], [], "experiment");
  invariant(input.protocol === V4_PROTOCOL, "unsupported v4 protocol");
  exactKeys(
    input.provenanceReuse,
    [
      "v3PreparedInputSha256",
      "v3TraceBundleSha256",
      "caseSourceSha256",
      "costStudyExcluded",
      "v3RankingExecuted",
  ...
  invariant(input.provenanceReuse.costStudyExcluded === true, "v4 must exclude v3 cost evidence");
  invariant(input.provenanceReuse.v3RankingExecuted === false, "v3 ranking must remain absent");
  invariant(
    input.provenanceReuse.v3PreparedInputSha256 ===
      "sha256:cb5e2a8f4c6799c07ff2ff988ab03c58e4b7bbcb2c878dd6313a5ffaefa6a94f" &&
      input.provenanceReuse.v3TraceBundleSha256 ===
        "sha256:48270861e414bb2f92fc9963e4dc87dad7c7d44919473708fd25740d363639a8" &&
      input.provenanceReuse.caseSourceSha256 ===
        "sha256:d2a19efd67058d54d250371860a2ec08af07eebc744a393dff00600b9f6cd6e5",
    "v4 provenance-reuse hash mismatch",
  );
...
export function evaluateV4Experiment(input) {
  const { repositories } = validateV4Experiment(input);
