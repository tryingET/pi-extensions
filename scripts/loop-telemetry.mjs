...
//   node scripts/loop-telemetry.mjs --json     # machine-readable
//   node scripts/loop-telemetry.mjs --failures # ranked failure causes
// Exit codes: 0 ok, 1 = repeat-failure alarm (same cause >= 5 runs), 2 = tool failure.
...
function parseRun(file, raw) {
  try {
    ...
    return { file, loop: "?", status: "unparseable", durationMin: null, attempts: 0, resumes: 0, failedPhase: null, failureCause: `parse error: ${error.message?.slice(0, 80)}` };
  }
}
...
if (asJson) {
  console.log(JSON.stringify({ total: runs.length, byStatus, byLoop, rankedCauses, alarmCauses }, null, 2));
  ...
  console.log(`loop-telemetry: ${runs.length} runs`);
