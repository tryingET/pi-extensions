// ---
// summary: "Tests neutral fake-npm restoration and fail-closed completion holds."
// read_when:
//   - "Changing canary host restoration, npm isolation, or retained integrity evidence."
// ---
...
import { ROOT, runJson, runJsonFailure } from "./restoration-fixture-commands.mjs";
import { assertHeldFailure, fixtureIdentity, fixtureSnapshot } from "./restoration-hold-test-helper.mjs";

test("compatibility canary restores temporary node_modules states with neutral fake npm", (t) => {
  ...
  function runHeldFailure(env, expected, verify) {
    const scenarios = JSON.parse(readFileSync(manifestPath, "utf8")).scenarios;
    const later = createScenarioPackage(`${scenarios[0].id}-hold-bystander`, false, lockedVersion);
    const before = fixtureSnapshot(later.packageDir);
    const marker = path.join(later.packageDir, "ran.marker");
    writeManifest([...scenarios, scenarioDefinition(`${scenarios[0].id}-hold-bystander`,
      [later.packagePath], later.packagePath,
      [process.execPath, "-e", `require("node:fs").writeFileSync(${JSON.stringify(marker)}, "ran")`])]);
    return runJsonFailure(["run", "--manifest", manifestPath], env, result => assertHeldFailure({
      result, root: ROOT, manifestPath, tempDir, env, ...expected,
      verify: held => {
        verify?.(held);
        assert.deepEqual(fixtureSnapshot(later.packageDir), before, "no later scenario or alignment effects");
      },
...
`,
  ...
  try {
    ...
    for (const initiallyPresent of [false, true]) {
      ...
      const marker = path.join(tempDir, `${id}-scenario-ran.marker`);
      const command = [process.execPath, "-e", `require("node:fs").writeFileSync(${JSON.stringify(marker)}, "ran")`];
      const packageIdentity = fixtureIdentity(malformed.packageDir);
      const nodeModules = path.join(malformed.packageDir, "node_modules");
      const originalIdentity = initiallyPresent ? fixtureIdentity(nodeModules) : null;
      const sentinelPath = path.join(nodeModules, "unrelated-sentinel.txt");
      const sentinelBefore = initiallyPresent ? fixtureSnapshot(sentinelPath) : null;
      writeManifest([scenarioDefinition(id, [malformed.packagePath], malformed.packagePath, command)]);
      const malformedResult = runHeldFailure(
        ...
        }), { calls: [[malformed.packageDir, "install"]] }, held => {
          const target = held.targets[0];
          assert.equal(target.initialNodeModules.kind, initiallyPresent ? "directory" : "absent");
          assert.equal(target.state, "alignment-intent");
          assert.equal(held.completionHold.effectMayBeActive, false);
          assert.deepEqual(fixtureIdentity(malformed.packageDir), packageIdentity);
          assert.deepEqual(fixtureIdentity(nodeModules), originalIdentity ?? target.ownedNodeModulesIdentity);
          assert.equal(readFileSync(path.join(nodeModules, ...hostPackages[0].split("/"), "package.json"), "utf8"), "{");
          for (const name of hostPackages.slice(1)) assert.equal(
            JSON.parse(readFileSync(path.join(nodeModules, ...name.split("/"), "package.json"), "utf8")).version, targetVersion);
          if (initiallyPresent) assert.deepEqual(fixtureSnapshot(sentinelPath), sentinelBefore);
          assert.equal(existsSync(marker), false, "preparation failure must block the current scenario");
        },
      );
    ...
    const mixedVictimBefore = fixtureSnapshot(mixedVictim);
    const mixedOriginalIdentity = fixtureIdentity(path.join(mixed.packageDir, "node_modules"));
    const mixedResult = runHeldFailure(
      fakeNpmEnv(mixedLog, {
      ...
      }), { calls: [[mixed.packageDir, "install"], [mixed.packageDir, "install"]],
        restoration: "failed", reason: "restoration-completion-unverified", childEffect: "restore-host" },
      () => {
        assert.deepEqual(fixtureSnapshot(path.join(mixed.packageDir, "node_modules")), mixedVictimBefore);
        assert.deepEqual(fixtureIdentity(path.join(tempDir, "mixed-original-node-modules")), mixedOriginalIdentity);
        assert.equal(readFileSync(path.join(tempDir, "mixed-original-node-modules", "unrelated-sentinel.txt"), "utf8"), "preserve me\n");
      },
    );
    assert.match(mixedResult.results[0].host.restoration.error, /target tree identity changed/);
    ...
    const npmSwapVictimBefore = fixtureSnapshot(npmSwapVictim.packageDir);
    const npmSwapLaterBefore = fixtureSnapshot(npmSwapLater.packageDir);
    const npmSwapResult = runHeldFailure(
      ...
      }), { calls: [[npmSwapEarly.packageDir, "install"]] }, held => {
        assertInstalledHostVersions(npmSwapEarly.packageDir, targetVersion);
        assert.deepEqual(fixtureIdentity(path.join(npmSwapEarly.packageDir, "node_modules")), held.targets[0].ownedNodeModulesIdentity);
        assert.deepEqual(fixtureSnapshot(npmSwapLater.packageDir), npmSwapVictimBefore);
        assert.deepEqual(fixtureSnapshot(npmSwapBackup), npmSwapLaterBefore);
      },
    );
    assert.match(npmSwapResult.results[0].lifecycleErrors.preparation, /identity changed/);
    ...
    const removedTargetResult = runHeldFailure(
      ...
      }), { calls: [[removedTargetEarly.packageDir, "install"]] }, held => {
        assertInstalledHostVersions(removedTargetEarly.packageDir, targetVersion);
        assert.deepEqual(fixtureIdentity(path.join(removedTargetEarly.packageDir, "node_modules")), held.targets[0].ownedNodeModulesIdentity);
        assert.equal(existsSync(removedTargetLater.packageDir), false, "no attempt to recreate the removed target");
      },
    );
    assert.match(removedTargetResult.results[0].lifecycleErrors.preparation, /integrity|exist|ENOENT/);
    assert.equal(
    ...
    const crossIdentity = fixtureIdentity(path.join(crossEarlier.packageDir, "node_modules"));
    const crossResult = runHeldFailure(
      fakeNpmEnv(path.join(tempDir, "cross-target.jsonl"), {
        ...
        FAKE_NPM_MUTATE_OTHER_PACKAGE: hostPackages[0],
      }), { calls: [[crossLater.packageDir, "install"]], reason: "scenario-integrity-failed", childEffect: null }, held => {
        assert.deepEqual(fixtureIdentity(path.join(crossEarlier.packageDir, "node_modules")), crossIdentity);
        for (const [index, name] of hostPackages.entries()) assert.equal(
          JSON.parse(readFileSync(path.join(crossEarlier.packageDir, "node_modules", ...name.split("/"), "package.json"), "utf8")).version,
          index === 0 ? "0.0.0" : targetVersion, "retain cross-target corruption, do not restore over evidence");
        assert.equal(readFileSync(path.join(crossEarlier.packageDir, "node_modules", "unrelated-sentinel.txt"), "utf8"), "preserve me\n");
        assertInstalledHostVersions(crossLater.packageDir, targetVersion);
        assert.deepEqual(fixtureIdentity(path.join(crossLater.packageDir, "node_modules")), held.targets[1].ownedNodeModulesIdentity);
        assert.equal(existsSync(crossMarker), false);
      },
    );
    assert.match(crossResult.results[0].lifecycleErrors.scenarioException, /align/);
    ...
    for (const replacementKind of ["symlink", "directory"]) {
      ...
      const cleanupVictimBefore = fixtureSnapshot(cleanupVictim);
      const cleanupResult = runHeldFailure(
        fakeNpmEnv(path.join(tempDir, `cleanup-${replacementKind}.jsonl`)),
        { calls: [[cleanupSwap.packageDir, "install"]], childEffect: "scenario" }, () => {
          assert.equal(lstatSync(cleanupNodeModules).isSymbolicLink(), replacementKind === "symlink");
          assert.deepEqual(fixtureSnapshot(replacementKind === "symlink" ? cleanupVictim : cleanupNodeModules), cleanupVictimBefore);
        },
      );
      assert.equal(cleanupResult.results[0].exitCode, 0, "successful command still requires integrity verification");
      assert.match(cleanupResult.results[0].lifecycleErrors.scenario.error,
        replacementKind === "symlink" ? /symlink/ : /aligned host package versions changed/);
    ...
    const swapVictimBefore = fixtureSnapshot(swapVictim.packageDir);
    const swapSourceIdentity = fixtureIdentity(swapSource.packageDir);
    const swapTreeIdentity = fixtureIdentity(path.join(swapSource.packageDir, "node_modules"));
    const swapResult = runHeldFailure(
      fakeNpmEnv(path.join(tempDir, "swap.jsonl")),
      { calls: [[swapSource.packageDir, "install"]], childEffect: "scenario" }, () => {
        assert.deepEqual(fixtureSnapshot(swapSource.packageDir), swapVictimBefore);
        assert.deepEqual(fixtureIdentity(swapBackup), swapSourceIdentity);
        assert.deepEqual(fixtureIdentity(path.join(swapBackup, "node_modules")), swapTreeIdentity);
        assertInstalledHostVersions(swapBackup, targetVersion);
        assert.equal(readFileSync(path.join(swapBackup, "node_modules", "unrelated-sentinel.txt"), "utf8"), "preserve me\n");
      },
    );
    assert.equal(swapResult.results[0].exitCode, 0);
    assert.match(swapResult.results[0].lifecycleErrors.scenario.error, /identity changed/);
    assert.equal(
    ...
    const removedPackageBefore = fixtureSnapshot(removedPresent.packageDir);
    delete removedPackageBefore.entries.node_modules; // The scenario removes only this tree.
    const removedResult = runHeldFailure(
      fakeNpmEnv(path.join(tempDir, "removed-present.jsonl")),
      { calls: [[removedPresent.packageDir, "install"]], childEffect: "scenario" }, () => {
        assert.equal(existsSync(path.join(removedPresent.packageDir, "node_modules")), false, "no recreation after completion failure");
        assert.deepEqual(fixtureSnapshot(removedPresent.packageDir), removedPackageBefore);
      },
    );
    assert.equal(removedResult.results[0].exitCode, 0);
    assert.match(removedResult.results[0].lifecycleErrors.scenario.error, /aligned node_modules is not a real directory/);

    for (const logPath of observedLogPaths) {
      t.diagnostic(`fake npm ${path.basename(logPath)}: ${fakeNpmCalls(logPath).map(call => `${path.basename(call.cwd)}:${call.operation}`).join(",") || "no calls"}`);
      assert.ok(
