...
The package keeps `self`, `interview`, `dispatch_subagent`, `session_closeout`, `intercom`, Prompt Vault read tools (`vault_query`, `vault_retrieve`, `vault_vocabulary`, `vault_dispatch_check`), the lightweight context planning tool (`context_plan`), pi-little-helpers launch tools (`fork_peer_spawn`, `scout_peer_spawn`, `candidate_peer_spawn`, `fresh_handoff_spawn`), the visible-loop checkpoint fallback (`visible_loop_child_complete`), the orchestrator loop dispatcher (`loop_execute`), and `toolbox` as foundational always-active custom tools while letting heavier package-owned tools and Prompt Vault diagnostics/mutations remain latent until explicitly activated. Current behavior:
...
The package-owned production bundles are `vault`, `context-packer`, `sci`, `ontology`, `designmd`, `autoresearch`, `orchestrator`, `agent_vent`, `subscription-resets`, and `peer-spawn`. Their tools must be registered by the owning package's Pi extension entry before toolbox activation; toolbox activation only changes the active set. The `context-packer` read profile includes packet planning, bounded packet assembly, packet-local dogfood receipt evaluation, and non-persistent aggregate summarization; it does not make dogfood observations canonical evidence.

The `orchestrator` bundle defaults to its `read` profile, which includes the owner-registered `direction_controller_readback` alongside bounded society, boundary-telemetry, and ontology context reads. D2E and direction-controller catalog searches expose the bundle, while read-focused recommendations select this profile without activating it. If any selected read-profile tool is not registered, Toolbox refuses the entire activation before changing the active set, recording leases, or queueing a continuation; enable/install `pi-society-orchestrator` and `/reload` or start a fresh session instead. Verified activation may independently restore a missing foundational always-active tool such as `loop_execute`; that baseline repair is not D2E read-profile membership or new workflow authorization. Selected-profile activation grants no AK, DSPx, dispatch, transition-apply, or workflow-execution authority and does not prove the owner tool's runtime semantics; those remain owned and validated by `pi-society-orchestrator`.

The `sci` bundle exposes only composite workflows, not SCI's primitive registry. Its `read` profile activates `explore_symbol_impact` and `locate_confirm_definition`. Its risk-gated `mutating` profile activates preview-only snapshot/check workflows whose Pi schemas omit apply; the gate remains necessary because caller-selected local check commands can have effects.
...
`subscription-resets` exposes the `read` profile with `subscription_resets`, registered by pi-little-helpers. Activate with `toolbox({ action: "activate", bundle: "subscription-resets" })`, then query one exact allowed provider. This grants no login, browser-store access, card grant or redemption capability. Missing app sign-in is a returned inventory state, not a reason to broaden credentials.
...
```text
active tools (22): read, bash, edit, write, self, interview, dispatch_subagent, intercom, vault_query, vault_retrieve, vault_vocabulary, vault_dispatch_check, fork_peer_spawn, scout_peer_spawn, candidate_peer_spawn, fresh_handoff_spawn, visible_loop_child_complete, context_plan, loop_execute, explore_symbol_impact, locate_confirm_definition, toolbox
