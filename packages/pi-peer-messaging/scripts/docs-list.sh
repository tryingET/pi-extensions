#!/usr/bin/env bash
# ---
# summary: resolves and runs the canonical documentation listing validator for this package
# read_when:
#   - validating package documentation or troubleshooting docs-list resolution
# ---
set -euo pipefail
...
Defaults to strict package-doc validation when no args are provided.
...
Examples:
  ...
  ./scripts/docs-list.sh --docs docs --json
USAGE
}
...
if [[ $# -eq 0 ]]; then
  exec node "$SCRIPT_PATH" --docs docs --strict
fi
