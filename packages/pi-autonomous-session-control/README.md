...
- The helper now mirrors those startup/execution deadlines independently, pauses raw Pi reads when filtered protocol stdout is backpressured, tears down the raw process group on deadline/signal, and force-exits after a bounded grace when its parent never drains output. This prevents parent transport abandonment from keeping a completed or timed-out helper alive indefinitely.
- Shared capacity leases are scoped to the current repository session root and store dispatch/attempt identity plus an exact-token spawn-commit marker. Owned running sidecars add helper PID/start identity and detached raw-child PID/start/process-group custody.
- Recovery is effect-safe rather than helper-death-only: a live raw child or process-group member blocks reclaim; exact sidecar-backed reclaim requires raw-child death and kernel-observed group quiescence, and missing post-spawn custody remains fail-closed. Proven dead-owner pre-spawn leases and legacy dead-parent leases retain bounded recovery.
