...
The decision-budget fields remain on `self.evolution_candidate.v1`; the reflection guard supplies the closeout brake when analysis risks becoming self-ratifying. Future slices may improve ergonomics or session-local detection signals, but unresolved repetition must stay required-before-completion until an explicit positive external check is named.
