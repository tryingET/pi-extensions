...
- a dormant detached raw supervisor whose helper acquires the same per-slot transition fence as stale takeover, revalidates the exact lease, and then publishes exact custody immutably before the durable spawn marker, synchronous `raw_child_spawn_intent`, and start gate; takeover and raw start cannot pass each other when the parent dies inside the handoff
...
- custody-mode compatibility is fail-closed: old parent to additive new helper remains supported, while new `helper_owned` parent to old helper produces `capacity_release_deferred`; source-path update/rollback requires `/reload` or a fresh process to restore parent/helper version coherence
- helper-owned managed-process-group shutdown on abort/timeout and foreground-supervisor close plus helper-local startup/execution deadlines; protocol stdout and forwarded stderr each honor backpressure by pausing the corresponding raw stream, one positive watchdog covers both under `timeout=0`, the helper monitors exact parent identity, and custody-pipe loss makes the still-live supervisor leader terminate the complete managed group after helper death or `SIGKILL`, without a post-reap numeric PGID signal
...
- cross-process rate-limit results include a bounded, token-free holder projection (slot, unreadable lease, session or legacy candidates, age, parent/helper/raw PIDs and process state) and identify the capacity scope as this repository session root
- lease, custody, reclaim, and shared-limit payloads are fully written to private inodes before no-replace hard-link publication, session status uses atomic replacement, stale takeover/release uses identity-bearing compare/delete claims, and helper start shares the takeover transition fence; spawn markers and custody records are removed only after exact lease deletion succeeds
