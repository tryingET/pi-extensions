...
- **Cross-extension discoverability truth** — default checks keep parser/unit prompt-vault contract coverage, while `npm run test:live:prompt-vault` opts into `tests/prompt-vault-cross-extension-live.test.mjs` and proves the real `vault_query`/`vault_retrieve` registration path stays coherent with ASC-owned prompt provenance on `dispatch_subagent`, so exposed-tool discoverability and execution-boundary truth do not drift apart.
