...
- **Cross-extension discoverability truth** — parser/readiness coverage in `tests/prompt-vault-cross-extension-live.test.mjs` stays in default checks, while host-dependent live `vault_query`/`vault_retrieve` execution is opt-in via `npm run test:live:prompt-vault` (`ASC_RUN_LIVE_PROMPT_VAULT_TESTS=1`). The live path proves the real registration chain stays coherent with ASC-owned prompt provenance on `dispatch_subagent` without making default CI depend on local vault-client/Dolt readiness.
