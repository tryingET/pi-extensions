...
Create commits for explicitly authorized pi-typescript-tool changes only.
Do not broaden the task, stage unrelated root/package changes, or treat this
package as a git root. If commit authority is absent, report that boundary and stop.
Run gates with PI_EXTENSIONS_TMPDIR="$TMPDIR" to preserve managed scratch.
