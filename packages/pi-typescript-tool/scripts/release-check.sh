...
RELEASE_HOME="$(mktemp -d "$TMPDIR/pi-extension-release-home.XXXXXX")"
RELEASE_NPM_CACHE="$(mktemp -d "$TMPDIR/pi-extension-release-npm-cache.XXXXXX")"
TEST_AGENT_DIR=""
TEST_NPM_PREFIX=""
TEST_NPM_CACHE=""
TARBALL_PATH=""
...
if [[ ! -f "$MONOREPO_ROOT/scripts/npm-pack-json.mjs" ]]; then
  echo "Monorepo npm pack normalizer is missing: $MONOREPO_ROOT/scripts/npm-pack-json.mjs" >&2
  exit 1
fi
...
echo "$PACK_JSON"
PACK_JSON="$(printf '%s' "$PACK_JSON" | node "$MONOREPO_ROOT/scripts/npm-pack-json.mjs")"
...
# Private mode validates the real artifact without publication probes or Pi install.
if [[ "$(node -p 'JSON.parse(require("node:fs").readFileSync("package.json", "utf8")).private === true')" == "true" ]]; then
  echo "== private artifact: npm pack + provider-free load/execution (no publish/install)"
  TARBALL="$(isolated_npm pack --silent | tail -n 1)"
  TARBALL_PATH="$ROOT_DIR/$TARBALL"
  node ./scripts/pack-smoke.mjs "$TARBALL_PATH"
  echo "Private artifact checks passed; live Pi behavior not verified."
  exit 0
fi
