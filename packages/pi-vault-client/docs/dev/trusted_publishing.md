---
summary: "Trusted publishing notes for monorepo package component pi-vault-client."
read_when:
...
- Package release is component-scoped.
- Publish workflow uses npm OIDC trusted publishing.

## Package-level requirements

- `package.json.repository.url` must point to monorepo git URL.
- `package.json.repository.directory` must match `packages/pi-vault-client`.
...
- Package passes `npm run release:check:quick`.
- Root release workflow can produce/update component release PR.
- Publish workflow completes with `npm publish --provenance --access public` for the package.
