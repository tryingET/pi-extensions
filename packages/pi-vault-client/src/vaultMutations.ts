...
type MutationActorContextResult =
  | { status: "ok"; actorCompany: string; source: string }
  | { status: "error"; message: string };

type TemplateMutationRecord = Pick<
  Template,
  | "content"
  | "artifact_kind"
  | "control_mode"
...
export interface TemplateMutationDependencies {
  contracts: GovernedContracts;
  ...
  getActiveTemplateByName: (name: string) => Template | null;
}
...
function validateCompanyList(companies: string[], contracts: GovernedContracts): string | null {
...
function validateTemplateRecord(
  template: TemplateMutationRecord,
  contracts: GovernedContracts,
): string | null {
...
export function insertTemplate(
  ...
  dependencies: TemplateMutationDependencies,
): InsertResult {
  ...
  const existing = dependencies.queryVaultJson(
  ...
  const templateId = dependencies.queryVaultJson(
    `SELECT id FROM prompt_templates WHERE name = '${escapedName}'`,
...
export function updateTemplate(
  ...
  const visibilityCompaniesJson = dependencies.escapeSql(
    JSON.stringify(merged.visibility_companies),
  );
