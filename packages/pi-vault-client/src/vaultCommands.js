...
export function registerVaultCommands(pi, runtime, receipts, dispatchRuntime) {
    if (receipts) {
        pi.on("message_end", async (event, ctx) => {
            ...
            if (finalized.status === "no-match" || finalized.status === "matched")
                return;
            const warning = finalized.status === "rejected"
    ...
    pi.on("input", async (event, ctx) => {
        ...
        const isVaultInput = /^\/(vault(?::|\s|$)|vault-search\b|route\b|next-10-expert-suggestions\b)/.test(text);
        if (isVaultInput) {
        ...
        if (text === "/vault-search" || text.startsWith("/vault-search ")) {
            ...
            if (!query) {
                if (ctx.hasUI)
                    ...
                    return { action: "transform", text: "Usage: /vault-search <query>" };
                return { action: "handled" };
            }
            const companyContext = resolveCommandCompanyContext(runtime, ctx);
            if (!companyContext.ok) {
    ...
    pi.registerCommand("vault", {
        ...
        handler: async (args, ctx) => {
            ...
            if (!guarded.ok)
                return ctx.ui.notify(guarded.error, "warning");
            const preparedPrompt = queuePreparedExecution(receipts, {
                queued_at: new Date().toISOString(),
                invocation: {
                    surface: "/vault",
