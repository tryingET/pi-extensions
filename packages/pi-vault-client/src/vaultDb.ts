...
import {
  ...
  DOLT_TELEMETRY_LIMIT,
  type DoltExecutionEnvironment,
  type DoltJsonResult,
  type DoltTelemetryEvent,
  type DoltTelemetryStats,
...
function createDoltTelemetryState() {
  return {
    ...
    tempSourceCounts: {} as Record<string, number>,
  };
}
...
function classifyDoltCommand(args: string[]): { command: string; argsPreview: string } {
...
function extractExecExitCode(error: unknown): number | null | undefined {
...
function recordDoltTelemetryEvent(
  event: Omit<DoltTelemetryEvent, "timestamp"> & { timestamp?: string },
): void {
  const normalized: DoltTelemetryEvent = {
  ...
  if (normalized.success) doltTelemetry.successCount += 1;
  else doltTelemetry.failureCount += 1;
...
function listRecentDoltTelemetry(limit = 20): DoltTelemetryEvent[] {
  const normalizedLimit = Math.max(
    1,
    Math.min(Math.floor(Number(limit) || 20), DOLT_TELEMETRY_LIMIT),
  );
  return doltTelemetry.events.slice(-normalizedLimit);
}

function getLatestDoltTelemetryFailure(): DoltTelemetryEvent | null {
...
function getDoltTelemetryStats(): DoltTelemetryStats {
  return {
    ...
    averageLatencyMs:
      doltTelemetry.totalCalls > 0 ? doltTelemetry.totalLatencyMs / doltTelemetry.totalCalls : 0,
...
function summarizeDoltTelemetry(): string {
...
function shouldEmitVaultDiagnosticLogs(): boolean {
  const value = process.env.PI_VAULT_LOG_ERRORS?.trim().toLowerCase();
  return value === "1" || value === "true" || value === "yes" || value === "on";
}

function emitVaultDiagnostic(label: string, error: unknown): void {
  if (!shouldEmitVaultDiagnosticLogs()) return;
  console.warn(`${label}: ${formatVaultError(error)}`);
}

function getActiveVaultDir(): string {
  return process.env.VAULT_DIR?.trim() || VAULT_DIR;
}

function normalizeTempDirPath(tempDir: string): string {
  return path.resolve(String(tempDir || "").trim());
}

function dedupeTempDirCandidates(
  candidates: Array<{ source: string; path: string; create: boolean }>,
): Array<{ source: string; path: string; create: boolean }> {
...
function buildDoltTempDirCandidates(): Array<{ source: string; path: string; create: boolean }> {
  ...
  return dedupeTempDirCandidates(
    ...
    ].filter((candidate): candidate is { source: string; path: string; create: boolean } =>
      Boolean(candidate),
    ),
  );
}

function assertWritableDirectory(dirPath: string): void {
  accessSync(dirPath, constants.R_OK | constants.W_OK | constants.X_OK);
}

function findNearestExistingDirectory(dirPath: string): string {
...
function probeDoltTempDir(
  candidate: { source: string; path: string; create: boolean },
...
function formatDoltExecutionEnvironmentError(
  attempts: DoltExecutionEnvironment["attempts"],
  probeMode: DoltExecutionEnvironment["probeMode"],
): string {
...
function resolveDoltExecutionEnvironment(
  options: { probeMode?: DoltExecutionEnvironment["probeMode"] } = {},
): DoltExecutionEnvironment {
  const probeMode = options.probeMode ?? "prepare";
  const attempts: DoltExecutionEnvironment["attempts"] = [];
...
function readHostDoltGlobalConfig(): Record<string, string> {
...
function ensureDoltProcessHome(tempDir: string): string {
...
function buildDoltProcessEnv(tempDir: string): NodeJS.ProcessEnv {
  return {
    ...process.env,
...
function logRetrievalBatch(entries: VaultRetrievalEntry[], context: VaultRetrievalContext): void {
  ...
  try {
    ...
    try {
      const before = db.prepare("PRAGMA user_version;").get() as
        | { user_version?: number | bigint }
        | undefined;
      const currentVersion = Number(before?.user_version ?? 0);
      if (currentVersion > ANALYTICS_SCHEMA_VERSION) {
        throw new Error(
          `analytics.db schema v${currentVersion} is newer than supported v${ANALYTICS_SCHEMA_VERSION}`,
        );
      }
      if (currentVersion !== 0 && currentVersion < ANALYTICS_SCHEMA_VERSION) {
        throw new Error(
          `analytics.db schema v${currentVersion} requires an explicit migration to v${ANALYTICS_SCHEMA_VERSION}`,
        );
      }
