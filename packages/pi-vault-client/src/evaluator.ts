...
interface StoredPromptVariantV1 {
  schema_version: 1;
...
// ============================================================================
// VAULT OPERATIONS (kept for API compatibility with extension wiring)
// ============================================================================
...
const PROMPT_EVAL_CONTEXT_ERROR =
  "Explicit company context is required for prompt_eval variants. Set PI_COMPANY or invoke the tool from a company-scoped cwd.";

function formatEvaluatorError(error: unknown): string {
  return error instanceof Error ? error.message : String(error);
}

function getPromptVariantStorePath(): string {
...
function ensurePromptVariantStoreDir(filePath: string): void {
  mkdirSync(path.dirname(filePath), { recursive: true });
}

function normalizeStoredPromptVariant(value: unknown): StoredPromptVariantV1 | null {
  if (!value || typeof value !== "object") return null;
  const candidate = value as Record<string, unknown>;
  if (candidate.schema_version !== 1) return null;
  if (candidate.record_kind !== "prompt_eval_variant") return null;
  if (typeof candidate.id !== "string" || !candidate.id.trim()) return null;
  if (typeof candidate.name !== "string" || !candidate.name.trim()) return null;
  if (typeof candidate.content !== "string") return null;
  if (typeof candidate.description !== "string") return null;
  if (typeof candidate.company !== "string" || !candidate.company.trim()) return null;
...
function sortStoredVariantsNewestFirst(
  left: StoredPromptVariantV1,
  right: StoredPromptVariantV1,
): number {
  return Date.parse(right.created_at) - Date.parse(left.created_at);
}

function readStoredPromptVariants(filePath = getPromptVariantStorePath()): StoredPromptVariantV1[] {
  ...
  try {
    ...
    return raw
      ...
      .filter((record): record is StoredPromptVariantV1 => Boolean(record))
...
function toPromptVariant(record: StoredPromptVariantV1): PromptVariant {
  return {
    id: record.id,
    name: record.name,
...
function listVariantsForCompany(
  company: string,
  filePath = getPromptVariantStorePath(),
): PromptVariant[] {
  const deduped = new Map<string, StoredPromptVariantV1>();
  for (const record of readStoredPromptVariants(filePath)) {
    if (record.company !== company) continue;
...
function createVariant(
  ...
  const record: StoredPromptVariantV1 = {
    schema_version: 1,
    record_kind: "prompt_eval_variant",
    id: variant.id,
    name: variant.name,
  ...
  const persisted = listVariantsForCompany(company, filePath).find(
...
function normalizeEvaluatorCwd(ctx: unknown): string | undefined {
  const cwd = (ctx as { cwd?: unknown } | undefined)?.cwd;
  return typeof cwd === "string" && cwd.trim() ? cwd.trim() : undefined;
}

function resolvePromptEvaluatorCompanyContext(
  ctx: unknown,
  options?: { env?: NodeJS.ProcessEnv; processCwd?: string },
): { ok: true; company: string; source: string; cwd?: string } | { ok: false; error: string } {
...
function formatVariantList(variants: PromptVariant[]): string {
  return variants
    .map(
      (variant) => `- ${variant.name} (${variant.id}): ${variant.description || "no description"}`,
    )
    .join("\n");
}
...
function extractChatCompletionText(value: unknown): string {
  const data = value as { choices?: Array<{ message?: { content?: string } }> };
...
function parseJudgeScores(
  ...
  const normalizedScoreEntries = new Map<string, number>();
...
function truncateForJudgeNotes(value: string, max = 160): string {
...
function buildJudgePrompt(testCase: TestCase, generatedOutput: string): string {
  return [
...
async function runABTest(
  ...
  for (const result of results) {
    if (!variantScores[result.variantId]) variantScores[result.variantId] = [];
  ...
  const bestVariantName =
    variants.find((variant) => variant.id === bestVariant)?.name || bestVariant;
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (action === "run_test") {
        ...
        const variants = variantIds
          ...
          .filter((variant): variant is PromptVariant => Boolean(variant));
        ...
        const testCases: TestCase[] = testInputs.map((input, index) => ({
          id: `tc_${index}`,
          name: `Test ${index + 1}`,
          input,
          criteria,
