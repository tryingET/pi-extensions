...
function createVariant(vault, name, content, description) {
    ...
    const tableReady = vault.exec(`CREATE TABLE IF NOT EXISTS prompt_variants (
    ...
    const inserted = vault.exec(`INSERT INTO prompt_variants (id, name, content, description, created_at) VALUES ('${escapedId}', '${escapedName}', '${escapedContent}', '${escapedDesc}', NOW())`);
    if (!inserted) {
    ...
    const persisted = vault.queryJson(`SELECT id FROM prompt_variants WHERE id = '${escapedId}' LIMIT 1`);
...
function listVariants(vault) {
    const result = vault.queryJson("SELECT id, name, content, description FROM prompt_variants ORDER BY created_at DESC");
...
// ============================================================================
async function judgeWithLocalModel(variant, testCase, endpoint, model) {
    try {
...
// A/B TEST EXECUTION
// ============================================================================
async function runABTest(config, _vault, variants, testCases) {
    const testId = `test_${Date.now()}`;
    const results = [];
    for (const variant of variants) {
        for (const testCase of testCases) {
            const startTime = Date.now();
            const judgeResult = await judgeWithLocalModel(variant, testCase, config.localModelEndpoint, config.defaultModel);
    ...
    for (const r of results) {
        if (!variantScores[r.variantId])
            variantScores[r.variantId] = [];
...
// TOOL REGISTRATION
// ============================================================================
export function registerPromptEvaluatorTool(pi, config, vault) {
    pi.registerTool({
        name: "prompt_eval",
...
Use to systematically improve prompts through experimentation.`,
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            if (action === "create_variant") {
                ...
                const created = createVariant(vault, params.name, params.content, params.description);
                if (!created.ok) {
                    return {
            ...
            if (action === "run_test") {
                ...
                const testCases = testInputs.map((input, i) => ({
                    id: `tc_${i}`,
                    name: `Test ${i + 1}`,
                    input,
                    criteria,
...
// COMMANDS
// ============================================================================
export function registerPromptEvaluatorCommands(pi, config, vault) {
