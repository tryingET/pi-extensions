...
import {
  ...
  type DispatchPostureResult,
  type ExecutionBinding,
  type FrozenDispatchPolicy,
...
const VALID_EXECUTION_SURFACES = new Set<ExecutionSurface>([
  "vault_command",
  "live_trigger",
  "route",
  "grounding",
...
function hasValidControlledVocabulary(controlMode: string, value: unknown): boolean {
  ...
  for (const key of CONTROLLED_DIMENSIONS) {
    ...
    const allowed = CONTROLLED_VOCABULARY[key as keyof typeof CONTROLLED_VOCABULARY];
    if (key === "selection_principles") {
      if (
        !Array.isArray(candidate) ||
        candidate.length === 0 ||
        candidate.some((item) => typeof item !== "string" || !allowed.has(item as never))
      )
        return false;
    } else if (typeof candidate !== "string" || !allowed.has(candidate as never)) return false;
  }
  return true;
}
...
interface IssuedAuthorization {
  state: "issued" | "claiming" | "claimed" | "handed_off" | "failed";
  sealedText: string;
  value: Exclude<DispatchAuthorizationV1, { disposition: "blocked" }>;
  request: PreparedExecutionRequest;
}

const ownedDispatchRuntimes = new WeakSet<object>();

export function isVaultDispatchRuntime(value: unknown): value is VaultDispatchRuntime {
  return Boolean(value && typeof value === "object" && ownedDispatchRuntimes.has(value as object));
}
...
function strictTemplate(raw: Record<string, unknown>): Template | null {
  ...
  return {
    ...
    controlled_vocabulary: vocabulary as Template["controlled_vocabulary"],
    status: "active",
    export_to_pi: true,
  };
}

function blocked(reason: DispatchBlockReason, safeMessage: string): DispatchAuthorizationV1 {
  return {
    schema: "pi.vault.dispatch-authorization.v1",
    disposition: "blocked",
    reason,
...
function bindingBytes(binding: Readonly<ExecutionBinding>): Buffer {
...
function authorizeRequest(
  ...
  | Exclude<DispatchAuthorizationV1, { disposition: "blocked" }>
  | Extract<DispatchAuthorizationV1, { disposition: "blocked" }> {
  ...
  const subjects: DispatchSubjectIdentity[] = [];
  const postures: DispatchPostureResult[] = [];
  ...
  const preparation: PreparedIdentity = {
  ...
  const aggregate: DispatchAggregateIdentity = Object.freeze({
...
function revalidateIssuedAuthorization(
  runtime: DispatchRuntimeDeps,
  policy: FrozenDispatchPolicy,
  entry: IssuedAuthorization,
): boolean {
  ...
  if (templates.length !== names.length || templates.some((template) => template === null))
    return false;
  const byName = new Map((templates as Template[]).map((template) => [template.name, template]));
  const ordered = names
...
export function createVaultDispatchRuntime(
  ...
  const dispatchRuntime: VaultDispatchRuntime = {
    policy,
    async checkTemplates(templateNames, ctx = {}) {
      ...
      const rawRows = (result.value as { rows?: Record<string, unknown>[] }).rows ?? [];
      ...
      const results = (templates as Template[]).map((template) =>
        classifyDispatchPosture(template, policy),
      );
      if (
        results.some(
          (item) =>
