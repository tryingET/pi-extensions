...
function parseExtrasDsl(raw) {
    ...
    for (const part of parts) {
        ...
        if (!known.has(key))
            warnings.push(`Unknown DSL key ignored: ${key}`);
        else if (!value)
            warnings.push(`Empty DSL value for key: ${key}`);
        else
            map[key] = value;
    }
    if (freeform.length > 0 && !map.notes)
        map.notes = freeform.join(" | ");
    if (map.triggers && !map.frameworks)
        map.frameworks = map.triggers;
    return { map, freeform, warnings };
}
...
function shortlistFrameworks(objective, workflow) {
...
function discoverFrameworks(runtime, objective, workflow, currentCompany, limit = 5) {
    const text = `${objective} ${workflow}`.toLowerCase();
    const tokens = text
        .split(/[^a-z0-9-]+/)
        .filter((t) => t.length >= 4)
    ...
    const like = tokens
        .map((t) => `(name LIKE '%${runtime.escapeSql(t)}%' OR description LIKE '%${runtime.escapeSql(t)}%')`)
        .join(" OR ");
...
function resolveFrameworks(runtime, objective, workflow, dsl, currentCompany) {
    ...
    const exactCandidates = (overrideNames.length > 0 ? overrideNames : shortlistFrameworks(objective, workflow)).slice(0, 3);
    ...
    if (exactRetrieved.length >= 1) {
        return {
            ...
            value: {
                ...
                retrievalMethod: overrideNames.length > 0 && exactRetrieved.length < exactCandidates.length
                    ? "mixed"
                    : "exact",
                discoveryUsed: 0,
                invalidOverrides,
...
function buildFrameworkGroundingAppendix(selected, retrieval, dslWarnings, renderOptions) {
    ...
    if (dslWarnings.length > 0)
        appendix += `- dsl_warnings: ${dslWarnings.join(" | ")}\n`;
...
function buildGroundedNext10Prompt(runtime, commandText, options = {}) {
