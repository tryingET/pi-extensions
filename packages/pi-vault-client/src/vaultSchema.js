import { ANALYTICS_SCHEMA_VERSION, COMPATIBILITY_EPOCH_INTRODUCED_SCHEMA_VERSION, MAX_TESTED_SCHEMA_VERSION, MIN_SUPPORTED_SCHEMA_VERSION, REQUIRED_EXECUTION_COLUMNS, REQUIRED_FEEDBACK_COLUMNS, REQUIRED_PROMPT_TEMPLATE_COLUMNS, SUPPORTED_COMPATIBILITY_EPOCH, } from "./generatedPromptVaultContract.js";
export { REQUIRED_EXECUTION_COLUMNS, REQUIRED_FEEDBACK_COLUMNS, REQUIRED_PROMPT_TEMPLATE_COLUMNS, } from "./generatedPromptVaultContract.js";
...
function readActualContractVersions(queryVaultJson, actualVersion) {
    ...
    try {
        const result = queryVaultJson("SELECT compatibility_epoch, analytics_schema_version FROM schema_contract WHERE id = 1");
...
export function checkSchemaCompatibilityDetailed(queryVaultJson) {
    ...
    const versionStatus = actualVersion === null
        ? "unknown"
    ...
    const compatibilityEpochStatus = actualContract.epoch === null
        ? "unknown"
        : actualContract.epoch === SUPPORTED_COMPATIBILITY_EPOCH
            ? "compatible"
            : "mismatch";
    const analyticsSchemaStatus = actualContract.analytics === null
        ? "unknown"
    ...
    if (versionStatus === "newer_untested") {
        warnings.push(`Schema v${actualVersion} is newer than tested v${MAX_TESTED_SCHEMA_VERSION}; required structural contract is present.`);
    }
