...
function normalizeControlledVocabulary(value) {
...
function normalizeControlledVocabularyPatch(value) {
    if (!value || typeof value !== "object")
        return undefined;
    const raw = value;
    const normalized = {};
...
function buildTemplateUpdatePatch(params) {
    const patch = {};
    if (hasOwn(params, "content"))
        patch.content = String(params.content ?? "");
    ...
    if (hasOwn(params, "control_mode"))
        patch.control_mode = String(params.control_mode ?? "").trim();
    ...
    const controlledVocabularyPatch = normalizeControlledVocabularyPatch(params.controlled_vocabulary);
    if (controlledVocabularyPatch)
        patch.controlled_vocabulary = controlledVocabularyPatch;
    return patch;
}
function normalizeToolCwd(ctx) {
    const cwd = ctx?.cwd;
    return typeof cwd === "string" && cwd.trim() ? cwd.trim() : undefined;
}
const TOOL_READ_CONTEXT_ERROR = "Explicit company context is required for visibility-sensitive vault reads on the tool surface. Set PI_COMPANY or invoke the tool from a company-scoped cwd.";
function resolveToolExecutionContext(runtime, ctx) {
...
function resolveStrictToolReadExecutionContext(runtime, ctx) {
...
function buildToolMutationContext(ctx) {
    const cwd = normalizeToolCwd(ctx);
    return {
        ...(cwd ? { cwd } : {}),
...
function formatSchemaDiagnosticsReport(runtime, currentCompany, currentCompanySource, projectionResults) {
...
export function registerVaultDiagnosticsTool(pi, runtime) {
    pi.registerTool({
...
- vault_query({ controlled_vocabulary: { routing_context: ["review_followup"] }, include_governance: true })`,
        ...
        parameters: Type.Object({
            ...
            visibility_company: Type.Optional(Type.String({
                description: "Explicitly pin visible company; cross-company overrides are rejected on the tool surface",
            })),
            intent_text: Type.Optional(Type.String({ description: "Rank the candidate set against this intent text" })),
            controlled_vocabulary: Type.Optional(Type.Object({
            ...
            include_governance: Type.Optional(Type.Boolean({
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const filters = {
                ...
                visibility_company: typeof params.visibility_company === "string" && params.visibility_company.trim()
                    ? params.visibility_company.trim()
                    : undefined,
                intent_text: typeof params.intent_text === "string" && params.intent_text.trim()
            ...
            if (filters.visibility_company &&
                ...
                const error = "Cross-company visibility overrides are rejected on the tool surface. Set PI_COMPANY or invoke from the target company-scoped cwd instead.";
                return {
            ...
            const templatesResult = runtime.queryTemplatesDetailed(filters, limit, includeContent, executionContext);
            ...
            const gatedTemplates = dispatchPostures.filter((dp) => isOrchestratorGateRequired(dp.posture));
            let output = `# Vault Query Results (${templates.length})\n\n`;
            output += templates
                .map((template) => runtime.formatTemplateDetails(template, includeContent, { includeGovernance }))
                .join("\n\n---\n\n");
            if (gatedTemplates.length > 0) {
                output += "\n\n---\n\n## Dispatch Posture Summary\n\n";
                output += gatedTemplates
                    .map((dp) => `- **${dp.template_name}**: ${dp.posture}${dp.binding ? ` → ${dp.binding.execution_surface}(${JSON.stringify(dp.binding.execution_args)})` : ""}`)
                    .join("\n");
                output +=
        ...
        renderCall(args, theme) {
            const parts = [];
            const artifactKinds = args.artifact_kind || [];
            const controlModes = args.control_mode || [];
            const formalizationLevels = args.formalization_level || [];
            const ownerCompanies = args.owner_company || [];
            const visibilityCompany = args.visibility_company || "";
            const intentText = args.intent_text || "";
            const controlledVocabulary = args.controlled_vocabulary || {};
            if (artifactKinds.length > 0)
                parts.push(`kind:[${artifactKinds.slice(0, 2).join(", ")}]`);
            if (controlModes.length > 0)
                parts.push(`mode:[${controlModes.slice(0, 2).join(", ")}]`);
            if (formalizationLevels.length > 0)
                parts.push(`formal:[${formalizationLevels.slice(0, 2).join(", ")}]`);
            if (ownerCompanies.length > 0)
                parts.push(`owner:[${ownerCompanies.slice(0, 2).join(", ")}]`);
            if (visibilityCompany)
                parts.push(`visible:${visibilityCompany}`);
            if (intentText)
                parts.push(`intent:${intentText.slice(0, 24)}${intentText.length > 24 ? "..." : ""}`);
            const firstCv = Object.entries(controlledVocabulary).find(([, values]) => (values || []).length > 0);
            if (firstCv)
                parts.push(`cv:${firstCv[0]}=[${firstCv[1].slice(0, 2).join(", ")}]`);
            return new Text(theme.fg("toolTitle", theme.bold("vault_query ")) + theme.fg("accent", parts.join(" ")), 0, 0);
        },
...
Example: vault_retrieve({ names: ["inversion", "nexus"], include_content: true })`,
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const templatesResult = runtime.retrieveByNamesDetailed(names, includeContent, executionContext);
            if (!templatesResult.ok)
                return {
            ...
            const gatedNote = gatedTemplates.length > 0
        ...
        renderCall(args, theme) {
            ...
            return new Text(theme.fg("toolTitle", theme.bold("vault_retrieve ")) +
                theme.fg("accent", names.slice(0, 3).join(", ") + (names.length > 3 ? "..." : "")), 0, 0);
        },
        renderResult(result) {
            const text = result.content[0];
            return new Text(text?.type === "text"
...
- vault_insert({ name: "my-router", content: "...", artifact_kind: "procedure", control_mode: "router", formalization_level: "structured", owner_company: "core", visibility_companies: ["core", "software"], controlled_vocabulary: { routing_context: "review_followup", activity_phase: "post_review", input_artifact: "review_findings", transition_target_type: "framework_mode", selection_principles: ["constraint_preserving"], output_commitment: "exact_next_prompt" } })`,
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const controlledVocabularyRaw = params.controlled_vocabulary;
            const controlledVocabulary = controlledVocabularyRaw
                ? {
                    ...
                    selection_principles: normalizeStringArray(controlledVocabularyRaw.selection_principles),
                    output_commitment: controlledVocabularyRaw.output_commitment?.trim(),
                }
                : null;
            const result = runtime.insertTemplate(name, content, description, artifactKind, controlMode, formalizationLevel, ownerCompany, visibilityCompanies, controlledVocabulary, mutationContext);
        ...
        renderCall(args, theme) {
            return new Text(theme.fg("toolTitle", theme.bold("vault_insert ")) +
                theme.fg("accent", args.name || "?"), 0, 0);
        },
        renderResult(result) {
            const details = result.details;
...
- vault_update({ name: "my-router", description: "Refined router guidance", controlled_vocabulary: { selection_principles: ["constraint_preserving", "minimal_change"] } })`,
        ...
        parameters: Type.Object({
            ...
            artifact_kind: Type.Optional(Type.String({ description: "Updated ontology facet: artifact kind" })),
            control_mode: Type.Optional(Type.String({ description: "Updated ontology facet: control mode" })),
            formalization_level: Type.Optional(Type.String({ description: "Updated ontology facet: formalization level" })),
            owner_company: Type.Optional(Type.String({ description: "Updated governance owner company" })),
            visibility_companies: Type.Optional(Type.Array(Type.String(), { description: "Updated governance visibility boundary" })),
            controlled_vocabulary: Type.Optional(Type.Object({
                routing_context: Type.Optional(Type.String()),
                activity_phase: Type.Optional(Type.String()),
                input_artifact: Type.Optional(Type.String()),
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const patch = buildTemplateUpdatePatch(params);
        ...
        renderCall(args, theme) {
            return new Text(theme.fg("toolTitle", theme.bold("vault_update ")) +
                theme.fg("accent", args.name || "?"), 0, 0);
        },
        renderResult(result) {
            const details = result.details;
...
Example: vault_executions({ template_name: "nexus", limit: 10 })`,
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const executionContext = executionContextResult.value;
            const templateName = typeof params.template_name === "string" && params.template_name.trim()
        ...
        renderCall(args, theme) {
            const templateName = args.template_name || "recent";
            return new Text(theme.fg("toolTitle", theme.bold("vault_executions ")) + theme.fg("accent", templateName), 0, 0);
        },
        renderResult(result) {
            const details = result.details;
...
Example: vault_rate({ execution_id: 42, rating: 4, success: true, notes: "Found root cause quickly" })`,
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const receiptAuthorization = receipts?.readReceiptAuthorizationByExecutionId?.(executionId) || null;
            const result = runtime.rateTemplate(executionId, rating, params.success, params.notes || "", mutationContext, {
                executionReceipt: receiptAuthorization?.receipt || null,
                executionReceiptVerificationKeys: receiptAuthorization?.verificationKeys || [],
            });
            return {
                content: [{ type: "text", text: result.message }],
                details: { ok: result.ok, executionId, rating, success: params.success },
            };
        },
        renderCall(args, theme) {
            return new Text(theme.fg("toolTitle", theme.bold("vault_rate ")) +
                theme.fg("accent", `execution:${args.execution_id || 0} (${args.rating || 0}/5)`), 0, 0);
        },
        renderResult(result) {
            const details = result.details;
...
Example: vault_dispatch_check({ template_names: ["transcendent-iteration", "ooda"] })`,
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const templatesResult = runtime.retrieveByNamesDetailed(names, false, // content not needed for posture check
            executionContext);
            if (!templatesResult.ok) {
                return {
                    content: [
        ...
        renderCall(args, theme) {
            const names = args.template_names || [];
            return new Text(theme.fg("toolTitle", theme.bold("vault_dispatch_check ")) +
                theme.fg("accent", names.slice(0, 3).join(", ") + (names.length > 3 ? "..." : "")), 0, 0);
        },
        renderResult(result) {
            const details = result.details;
