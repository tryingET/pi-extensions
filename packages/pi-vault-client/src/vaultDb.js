...
function resolveCurrentCompanyContext(cwd) {
    if (process.env.PI_COMPANY)
        return { company: process.env.PI_COMPANY, source: "env:PI_COMPANY" };
    ...
    if (normalizedCwd.includes("/house/"))
        return { company: "house", source: `cwd:${effectiveCwd}` };
...
function validateCompanyList(companies, contracts = getContracts()) {
...
function validateControlledVocabulary(controlMode, controlledVocabulary, contracts = getContracts()) {
...
function validateTemplateRecord(template, contracts = getContracts()) {
...
export function prepareTemplateUpdate(name, existing, patch, contracts = getContracts()) {
    if (!existing)
...
function rateTemplate(executionId, rating, success, notes, context, options) {
  ...
  `);
    if (insertedRows == null)
        return { ok: false, message: "Failed to record feedback" };
    if (insertedRows !== 1) {
        return {
            ok: false,
