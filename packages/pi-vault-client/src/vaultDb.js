        ...
        try {
         ...
         VALUES ('template', ?, ?, ?, ?, ?, ?, ?)`);
            for (const entry of valid) {
                insert.run(Number(entry.templateId), Number.isFinite(entry.entityVersion) ? Number(entry.entityVersion) : null, tool, queryContext, Number.isFinite(entry.rank) ? Number(entry.rank) : null, resultCount, company.length > 0 ? company : null);
            }
        }
        finally {
            db.close();
