...
- Execution status: done (exit 0, 54110ms).
- Evidence write outcome: ak.
- Captured output excerpt: ## AXIOMS (Non-Negotiables) - **Wire owns decode identity**: The decoder must report the encountered extension ID; Rust cannot predeclare it as `PortRequest::id()` does at `custom_extension_v2.rs:65`. - **Recognition di…
