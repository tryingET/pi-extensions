...
- Execution status: done (exit 0, 45874ms).
- Evidence write outcome: ak.
- Captured output excerpt: ## BUGS - [env-coupled partial suite]: `scripts/run-normal-tests.sh:10` initializes `SLICE=${SLICE:-}` and later treats any non-empty `SLICE` as single-slice mode, while usage says single-slice mode is only `--slice K/N…
