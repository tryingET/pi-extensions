...
- Execution status: done (exit 0, 66475ms).
- Evidence write outcome: ak.
- Captured output excerpt: ## AXIOMS (Non-Negotiables) - **Capability must be explicit:** A script’s existence does not prove support for a `write-file` stage. - **Repository contracts own validation semantics:** `pi-extensions/scripts/quality-ga…
