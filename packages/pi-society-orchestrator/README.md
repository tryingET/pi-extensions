...
- Authorized finalizer output now carries `authorized_finalizer_cleanup_blockers` (lower is better, target `0`) to prove the accepted finalizer token did not authorize candidate cleanup or promotion; cleanup requires a separate `candidate_cleanup` token, and merge/release/promotion requires a separate `promotion` token.
- It remains plan/report only: no peer launch, benchmark, export, review, evidence write, merge, worktree lifecycle action, cleanup, or promotion is executed by the orchestrator.
