...
- `loop_execute`
- `vault_execute_template` (Prompt Vault dispatch bridge that executes known loop-bound templates through `loop_execute`, immutably binds exactly the Layer-12 software, repo holding, and execution-memory core D2E templates—to the proposal/applied `D2E_TRANSFER_COMPLETE_V1` workflow gate, and fails closed for every other workflow-grade/loop template without an execution binding)
