...
- operator-visible status surfaces should describe the orchestrator coordination role, the ASC execution seam, and routing scope without implying that orchestrator owns the execution runtime; the persistent footer may omit static routing when the startup notice and `/runtime-status` retain it
...
- `level4_autoresearch_campaign_runner` is the first genuine Level-4 automation layer above Level-3. It carries an `autoresearch.level4_prompt_runner_bundle.v1` that follows visible `candidate_peer_spawn` -> ACK/FINAL watches -> controller lineage verification -> bind/measure/export/review -> owner gates. Its `autoresearch.level4_post_integration_cleanup_ready.v1` packet validates exact registry-backed peer ids/worktrees/branches, but now emits only lifecycle-v2 `candidate_peer_closeout` status/plan handoffs. Historical `candidate_peer_cleanup` v1 execution and raw process/worktree/branch deletion commands are permanently withheld. The runner may automate explicitly allowed safe measurement/export/review steps, but finalizer apply, lifecycle-v2 disposition/archive/authorization/cleanup, AK writes, merge/release, and promotion remain separate owner surfaces.
- `review_matrix_campaign` includes `autoresearch.level3_review_selection_substrate.v1`, which aggregates controller-verified candidate-result packets into per-cell selection state and prepares validation/finalizer handoff without apply, AK-write, merge, promotion, or cleanup authority. Successful integration makes lifecycle-v2 closeout planning timely; it does not authorize deletion.
...
- `level3_authorized_finalizer_cleanup_plan` is the Slice-5 surface. It consumes the level-3 manifest plus reviewed candidate-wave/matrix packet refs, validates the post-fan-in finalizer contract, and accepts only an exact `finalize_post_fanin` token tied to task/cwd/manifest/review scope before emitting a finalizer apply command packet. Candidate closeout inputs must also include exact candidate peer run ids. The resulting `autoresearch.level3_candidate_lifecycle_closeout_handoff.v2` contains only lifecycle-v2 status/plan calls and no process, worktree, branch, or registry-v1 cleanup command. It reports finalizer/closeout blockers, keeps rollback receipts non-authoritative, blocks dirty/off-limits/stale review posture, and never treats successful integration or finalizer authorization as cleanup, promotion, or AK-write authority.
- Authorized finalizer output carries `authorized_finalizer_cleanup_blockers` (lower is better, target `0`) to prove the finalizer token authorizes neither promotion nor cleanup execution. Cleanup additionally requires lifecycle-v2 owner disposition, integration proof when accepted, verified archive, exact cleanup authorization, and unchanged resource bindings; merge/release/promotion and AK writes remain separate gates.
...
- `/runtime-status` also renders a read-only AK close-frame/readiness section when the current cwd has exactly one active strategic frame; if no active implementation wave exists, it renders DiscoveryOrExecution/default-discovery posture and marks reserved/non-active waves as non-execution paths instead of implying wave creation. The section displays route posture, common proceed rule, route policy/state machine, active task, active-task close-check readiness/warnings, closeout readiness, apply support, close-frame blockers, closeout-domain blockers, non-authorizations/non-actions, and safe read commands without running AK lifecycle writes.
...
- `loop_execute` now exposes bounded loop execution controls: loop-level timeout, optional per-phase timeout forwarding, phase start/progress/complete update streaming, and compact result details containing run identity, phase statuses, failure kinds, and artifact paths rather than full phase outputs. Known bootstrap/transport/protocol failures include one bounded message from a closed diagnostic taxonomy. Arbitrary child stderr remains private checkpoint evidence and is never publicly rendered through regex redaction.
- Every `loop_execute` phase projects the same logical run id plus exact phase metadata through ASC's bounded observation projector. When `pi-little-helpers` is present in an interactive Ghostty TUI, that produces one read-only observer for the whole run rather than one tab per phase. The orchestrator emits a final `group_terminal` only for a terminal lineage; `confirmed_no_effects` leaves an exactly retryable lineage nonterminal. Ghostty launch/rendering remain `pi-little-helpers` ownership, while ASC retains child execution, cancellation, settlement, and effect receipts.
- Routine phase dispatch no longer inherits the old short wall-clock cutoff. ASC and this adapter default to a four-hour per-dispatch emergency deadman, the whole loop defaults to 24 hours, and bounded heartbeat/semantic-activity telemetry supports healthy/quiet/stall supervision without treating silence as automatic cancellation authority. Explicit caller timeouts remain absolute overrides.
- Checkpointed runs can continue under the same run lineage with `resume_run_id`, `expected_failed_phase`, and `recovery_mode: "validate_then_retry"`, but only from an undispatched phase or an attempt explicitly recorded as having emitted no effects. Every phase attempt is checkpointed before hooks or dispatch. ASC durably issues an attempt-bound `asc.dispatch_effect_receipt.v1`; the Orchestrator adapter accepts it only when its private regular file remains inside the trusted ASC sessions root, its persisted bytes match the returned receipt, and its dispatch/attempt identifiers match ASC's public result. Missing, altered, escaped, linked, replayed, correlation-mismatched, or otherwise mismatched receipts remain `effect_indeterminate`. ASC emits `confirmed_no_effects` only for failures proven to occur before the child spawner is invoked or, for ASC's owned transport, before its synchronous raw-Pi-child spawn-intent marker, binding the Orchestrator loop-attempt correlation into the receipt; the loop skips phase evidence and exit-hook effects, publishes no terminal KES while that same lineage remains exactly retryable, and permits one exact same-lineage retry only when no phase-enter hook ran. Timed-out, crashed, aborted, or post-spawn failed attempts remain blocked from mechanical retry until their owning surfaces provide stronger reconciliation; repository and KES hashes alone are not treated as effect receipts. The runtime also fails closed on missing or expired checkpoints, stale/concurrent locks, objective/repository/phase-graph mismatch, non-linear history, or tracked/untracked Git-state drift. Private checkpoints live under `~/.pi/agent/state/pi-society-orchestrator/loop-runs/` in a seven-day rolling window based on last checkpoint activity. Each loop invocation scans at most 1,000 checkpoint entries and prunes up to 100 expired terminal (`done`, `failed`, or `aborted`) checkpoints while preserving `running`, live-locked, stale-locked, and malformed state; `/loop-checkpoints` previews retention and `/loop-checkpoints prune` applies it explicitly. KES diary/learning artifacts follow their separate owner retention policy and are not deleted with checkpoints. Legacy runs created before this contract deliberately remain non-resumable.
- The `transcendent` loop is aligned to Transcendent Iteration v4 (`Diagnose → 100x → 100x → Debt Targeting → Dissolve → Rebuild → Alien Pass → Closure Gate`) and defaults to fail-fast after failed or timed-out phases; callers must set `continue_after_failure: true` to opt into same-invocation resilient continuation, while cross-invocation continuation must use the validated resume contract. Closure is machine-gated: the closure phase must emit exactly one standalone `CLOSURE_GATE: PASS` or `CLOSURE_GATE: INCOMPLETE` verdict, so explanatory prose cannot silently turn an incomplete review into loop success.
...
- Prompt Vault execution dispatch now has an authorization-bound orchestrator bridge. `vault_execute_template` maps `transcendent-iteration` and `ooda` to `loop_execute`; its exact `deep-review.v1` binding prepares sealed Vault bytes through prompt-plane V2, claims/revalidates identity, persists a durable Vault handoff, injects those exact bytes into a one-reviewer `workflow_execute` run, and accepts completion only when ASC's private persisted effect receipt verifies as `settled` with that exact handoff correlation. Unknown or unbound workflow-grade templates still fail closed. Set `PI_VAULT_GATED_DISPATCH=0` for rollback-to-blocked; rollback never restores raw prompt execution.
- The same bridge keeps `layer12-040-direction-to-execution-ak-native` (software) and `repo-direction-to-execution` (holding) on the legacy `D2E_TRANSFER_COMPLETE_V1` gate; there is no `direction-to-execution` template binding. Core `execution-memory-transfer` is separately bound to `D2E_EXECUTION_MEMORY_V1`. That path is disabled unless `PI_ORCH_D2E_EXECUTION_MEMORY_MODE=enabled` and the exact immutable R4 binary path/hash are configured. It runs one `ak decision execution-memory-check ... --profile d2e-transfer-v1 --machine` invocation, validates the closed Decision 100 envelope, and returns only `D2E_EXECUTION_MEMORY_OBSERVATION_V1`. `ok=true` and `pre_execution_memory_ready=true` remain observation facts: applied readiness is always false, authorization remains negative-only, effects stay not-materialized, and no workflow/Vault/repository/task mutation path is reachable. Applied mode is structurally unsupported. Rollback disables the consumer before producer withdrawal; source tests do not constitute installed R6 dogfood.
...
- loop execution consumes this seam only at terminal outcomes: success emits one package-owned diary and at most one candidate-only learning digest from exactly one explicit `KES_CLAIM:` line; failure emits one compact diary tombstone; a signal aborted before execution emits nothing. Exact in-flight phase outputs and claim text remain in the private checkpoint; public terminal artifacts carry bounded attribution and objective/output/claim digests only. Owner-reported truncation fails closed before checkpoint replacement or publication
...
- the original KES packet and TG3 hardening slice remain landed; the newer terminal-bundle/checkpoint hardening is an implementation candidate pending exhaustive real-process interruption proof and fresh installed-runtime validation
...
`npm run release:check` now also proves the installed-package KES path and compact loop contract: a successful kaizen loop emits one terminal package-owned `diary/` bundle plus at most one explicitly claimed candidate-only `docs/learnings/` artifact under the true installed package root without returning full phase outputs, while the installed transcendent smoke proves fail-fast behavior after a timed-out first phase and streamed phase lifecycle updates.
