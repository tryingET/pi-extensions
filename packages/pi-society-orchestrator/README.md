...
- `/runtime-status` also renders a read-only AK close-frame/readiness section when the current cwd has exactly one active strategic frame and implementation wave; it displays route posture, active task, closeout readiness, apply support, blockers, non-actions, and safe read commands without running AK lifecycle writes.
