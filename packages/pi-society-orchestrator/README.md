...
- The session footer now uses prioritized slots: model + `orchestrator→ASC` stay primary, compact `DB`/`Vault` health badges appear only when width allows, and narrow widths drop those optional badges before sacrificing routing visibility.
