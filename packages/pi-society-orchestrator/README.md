...
- The session footer now uses prioritized slots: model + `orchestrator→ASC` render when width allows, compact `DB`/`Vault` health badges are optional, and narrow widths drop badges first, then the seam, before sacrificing routing visibility.
