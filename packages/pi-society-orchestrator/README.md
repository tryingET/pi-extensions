...
- Checkpoint and matrix-review reports now also render an `autoresearch.matrix_campaign_cockpit.v1` cockpit/dashboard with compact cell progress/posture rows, selected lane plus packet inventory visibility, per-cell and campaign next legal actions, dashboard-first owner route, no-hidden-execution/promotion boundaries, and `matrix_cockpit_blockers` target `0` proof coverage.
- It remains plan/report only: no peer launch, benchmark, export, review, evidence write, merge, worktree lifecycle action, or promotion is executed by the orchestrator.
