...
- The same bridge immutably binds `layer12-040-direction-to-execution-ak-native` (software), `repo-direction-to-execution` (holding), and `execution-memory-transfer` (core) to `D2E_TRANSFER_COMPLETE_V1`; there is no `direction-to-execution` template binding. Caller mode defaults to proposal. D2E proposal mode is lawful read-only success and returns exact template version/content, AK task-scope, and canonical task-intent digests. The intent digest binds the immutable live title/description plus done-contract required outcomes/validation/review questions and guardrails; a null description falls back to title plus that task-native contract rather than caller objective. Applied mode is disabled unless the controller sets `PI_ORCH_D2E_TRANSFER_MODE=enabled`; it requires both the invoking actor and current Pi session identity to equal the live AK claimant, re-reads exact packet/task/decision/scope/lease/deferral state after workflow preparation, then calls Prompt Vault `authorizePreparedExecution` and `claimPreparedExecution` immediately before dispatch. Proposal and failure receipts distinguish transfer-materialization authorization from separately required downstream-implementation authorization and return `not_ready` for required-packet blockage. `D2E_WORKFLOW_RESULT_V1` is the inner executor output; only verified materialization produces the outer `D2E_TRANSFER_COMPLETE_V1` receipt with `effect.disposition=materialized`. Typed `D2E_TRANSFER_FAILURE_V1` envelopes carry trusted sequencer phase, packet posture, transfer authorization-at-dispatch, separate downstream authorization, and effect disposition without error-code inference; final AK/Vault settlement failures remain `not_ready`/`indeterminate`, while preparation and initial packet failures remain `not_materialized`/`not_authorized`. Completion requires a new clean committed HEAD, exact in-scope changed paths, an exact machine output envelope, and unchanged authorized AK post-state—`status=done`, no-op, unrelated objectives, extra canonical task/decision links, identity drift, or another actor's lease all fail closed. Rollback sets `PI_ORCH_D2E_TRANSFER_MODE=disabled`, which preserves proposal readback and blocks every applied handoff. Source/package tests do not constitute installed activation proof; live enablement remains a controller closeout gate.
