...
- `loop_execute`
- `vault_execute_template` (Prompt Vault dispatch bridge that executes known loop-bound templates through `loop_execute` and fails closed for workflow-grade/loop templates without an execution binding; the Layer-12 D2E workflow routes explicitly to `direction_controller_readback` rather than pretending that its unavailable DSPx branch executed)
...
- Prompt Vault execution dispatch now has an orchestrator bridge: `vault_execute_template` uses `pi-vault-client/dispatch-runtime`, maps `transcendent-iteration` to `loop_execute(loop="transcendent")` and `ooda` to `loop_execute(loop="ooda")`, and fails closed for unknown loop/workflow-grade templates that lack an executable binding. Workflow-grade failures are operator/process gates: known package-owned procedures report their owner-specific lawful route instead of encouraging template-prose bypass. `layer12-040-direction-to-execution-ak-native` routes to the Pi-owned `direction_controller_readback` inspection tool, which composes AK `status`, `propose`, and `transition-check` readbacks, validates authority and no-apply invariants, and reports `program_availability` / `generated_program_dispatch_ready` without invoking or claiming DSPx execution.
