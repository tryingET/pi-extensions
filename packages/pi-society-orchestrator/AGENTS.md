...
- Direction authority stays at the monorepo root; when direction docs change or you need current posture, run `ak direction check` and `ak direction export` against the root repo context rather than inventing package-local direction state.
- Do not run `ak direction import` as a routine package-doc reconciliation step. It is a legacy migration/backfill path only, not the AK-native posture gate.
