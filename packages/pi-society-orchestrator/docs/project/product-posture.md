...
- Level-4 is explicitly **non-dispatching**: automation permissions never prove execution. V2 journal rows are identity/action-plan-bound controller cursor observations, not effect receipts; repeated waiting does not advance, terminal cursors persist, and legacy/drifting journals fail closed. Measured inventory requires owner packet validation and exact cell-scoped controller lineage, not existence or cockpit labels. Cell reviews gate on their own complete packet set; matrix/promotion handoffs require full verified fan-in. See [AK5582 repair and validation](2026-09-09-ak5582-level4-safety-repair.md).
...
- Level-4 post-integration closeout packets that validate candidate peer registry sidecars before naming exact lifecycle-v2 `candidate_peer_closeout` status/plan calls; registry-v1 cleanup and raw process/worktree/branch deletion commands remain permanently withheld;
...
- immutable owner-routed D2E bindings with no phantom `direction-to-execution` binding: `layer12-040-direction-to-execution-ak-native` (software) and `repo-direction-to-execution` (holding) retain the legacy `D2E_TRANSFER_COMPLETE_V1` materialization gate. Core `execution-memory-transfer` uses the separate default-disabled `D2E_EXECUTION_MEMORY_V1` consumer over one exact immutable AK Decision 100 machine invocation. It preserves owner projections without reconstructing AK truth, accepts only negative authorization states, always reports applied readiness false and no materialized effect, and has no workflow/Vault/repository/task mutation path. Package tests are not installed R6 dogfood.
...
Orchestrator owns the above-seam state model and report. `pi-autoresearch` owns measurement and candidate-result packets. `pi-little-helpers` owns visible candidate launch and lifecycle-v2 operational cleanup. Operators and target-owner surfaces own disposition, integration acceptance, promotion, and cleanup authorization.
...
The Level-4 closeout packet now includes `autoresearch.level4_post_fanin_promotion_handoff.v1` and `autoresearch.level4_post_integration_cleanup_ready.v1`. The promotion handoff is the anti-repeat packet: after Level-2/Level-3 fan-in is complete, Level-4 shows the remaining owner-gated sequence instead of starting another proof loop. It prepares owner review/finalizer-token-request guidance only from controller-verified measured packets and keeps finalizer apply, AK evidence, cleanup, and promotion as separate owner gates. The post-integration packet now emits only exact `candidate_peer_closeout({ action: "status" | "plan", peerRunIds })` lifecycle-v2 handoffs. It emits no executable `candidate_peer_cleanup` v1 call and no raw process termination, worktree removal, or branch deletion command. Lifecycle-v2 in `pi-little-helpers` remains the sole executable cleanup owner after disposition, integration proof when accepted, restoration-verified archive, and exact cleanup authorization.
