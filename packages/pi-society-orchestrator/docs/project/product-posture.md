...
- immutable Prompt Vault workflow bindings for `layer12-040-direction-to-execution-ak-native` (software), `repo-direction-to-execution` (holding), and `execution-memory-transfer` (core), with no phantom `direction-to-execution` binding: `vault_execute_template` proposal mode is lawful read-only success and returns template version/content plus exact task-scope and canonical task-intent digests. Applied mode is controller-disabled by default, requires the invoking actor and current Pi session identity to equal the live AK claimant, re-reads exact packet/task/decision/scope/lease/deferral state after preparation, and routes the exact template through Prompt Vault authorization/claim immediately before workflow dispatch. typed failure envelopes preserve sequencer phase, transfer authorization-at-dispatch, separate downstream authorization, and non-inferred effect disposition. `D2E_TRANSFER_COMPLETE_V1` additionally requires a new clean committed HEAD, exact in-scope changed paths, a closed machine output envelope, and unchanged AK post-state; package tests are not installed activation proof.
