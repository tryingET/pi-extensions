#!/usr/bin/env node
// summary: "Validates the deliberate private, skill-only pi-session-insights package contract."
// read_when:
...
const skill = readText("skills/pi-session-jsonl/SKILL.md");

for (const path of [
  "bin/pi-session-insights.mjs",
...
if (pkg.name !== "@tryinget/pi-session-insights")
  fail("package name must be @tryinget/pi-session-insights");
if (pkg.private !== true) fail("first slice must remain private/non-published");
if (!pkg.keywords?.includes("pi-package") || !pkg.keywords?.includes("pi-skill")) {
  fail("package keywords must include pi-package and pi-skill");
}
if (pkg.bin?.["pi-session-insights"] !== "./bin/pi-session-insights.mjs") {
  fail("package bin must expose ./bin/pi-session-insights.mjs");
}
if (pkg.pi?.skills?.length !== 1 || pkg.pi.skills[0] !== "./skills") {
  fail("package pi.skills must expose only ./skills");
}
if (pkg.pi?.extensions !== undefined || pkg.pi?.prompts !== undefined) {
  fail("skill-only first slice must not expose pi.extensions or pi.prompts");
}
if (pkg.peerDependencies !== undefined) {
  fail("jq-first skill-only package must not declare unused Pi runtime peer dependencies");
}
...
if (!skill.startsWith("---\n") || !/^name:\s*pi-session-jsonl\s*$/m.test(skill)) {
  fail("skill frontmatter must declare name: pi-session-jsonl");
}
if (!/^description:\s*\S+/m.test(skill)) fail("skill frontmatter must include a description");
...
const mode = fs.statSync("bin/pi-session-insights.mjs").mode & 0o111;
if (mode === 0) fail("bin/pi-session-insights.mjs must be executable");
...
console.log("pi-session-insights structure validation passed.");
