...
function validatePackageJson() {
  ...
  const requiredPeers = ["@earendil-works/pi-coding-agent", "typebox"];
  ...
  if (!Array.isArray(p.files) || p.files.length < 1) {
    ...
    for (const entry of ext) {
      const normalized = entry.replace(/^\.\//, "");
      const covered = p.files.some(
        (included) => normalized === included || normalized.startsWith(`${included}/`),
      );
      if (!covered) fail(`package.json files must cover extension artifact: ${normalized}`);
    }
  }
}
