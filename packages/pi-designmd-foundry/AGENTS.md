...
Expose readiness-verified DesignMD Foundry workflows to Pi:

- skill guidance for DESIGN.md-first UI/design work
- CLI-backed Pi tools for lint/export/import/palette/prompt operations
- no embedded LLM provider and no direct external design-tool claims beyond verified CLI/file behavior
...
## DesignMD adapter guardrails

- Keep DesignMD Foundry itself as the source of implementation logic; this package is an adapter over its CLI.
- Prefer read-only or artifact-producing tools. Avoid canonical `DESIGN.md` writes unless a later explicit safety design adds confirmation and fail-closed checks.
- External tool wrappers must stay honest about setup: OpenPencil, Pigmnts, and Penpot live paths are not assumed just because adapters exist.
- Do not add provider API keys or remote LLM calls here.
