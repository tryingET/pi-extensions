...
`context_pack` records explicit omissions instead of pretending full integration exists. Prompt Vault, AK, and FCOS adapters are planned provider seams. The current MVP assembles bounded AGENTS/docs-list/git/session sections plus SCI code context for safe path seeds, and for symbol seeds only inside a temporary sandbox bounded by at least one safe code path seed, when an SCI CLI is available.
...
`npm run check` includes structure validation, Biome, real TypeScript checking for the extension entrypoint, tests, and a quick package artifact check. For package-local production dogfood of real structured docs-list JSON intake, run `npm run dogfood:docs-list-json`; it assembles docs packets from package-root and package-subdirectory cwd values using real `rankedItems[].repoPath`, checks compact details redaction, and evaluates a non-persistent dogfood receipt.
