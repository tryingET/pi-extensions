...
`npm run release:check` includes isolated Pi tarball install plus runtime registration smoke for `/context-pack` and the four model-callable tools, then executes installed artifact code for `context_plan`, `context_pack`, and dogfood evaluation against a temporary read-only fixture.
