...
`context_pack` is the first bounded retrieval slice: it reads only local AGENTS files, caller-seeded or docs-list-ranked Markdown docs, trusted-system-git status, current Pi session/environment metadata, and SCI code context for caller-seeded code paths/symbols when an SCI CLI is available. It returns a curated Markdown packet as primary tool content and keeps structured details compact by default, omitting duplicated raw item content from `details`. It records omissions and a measurement receipt with estimated tool calls avoided, packet fill ratio, wired providers, already-loaded prompt dedupe, and provider gaps. It enforces the global packet budget across providers while preserving the configured reasoning reserve, preserves loader-style AGENTS order from root/ancestor files to deepest package files, and treats Markdown extensions case-insensitively. It does not mutate state. Caller-controlled path seeds are screened before provider planning; URI/drive-letter, absolute, parent-traversing, current-directory, hidden/internal, generated/vendor, or malformed path seeds are omitted from provider queries and surfaced as blocked path risks. Caller-controlled `cwd`/`repoRoot` values are also screened before they are echoed as workspace posture, and retrieval repeats workspace containment plus descriptor/TOCTOU checks before file content enters a packet. When no explicit safe Markdown seed is supplied, the docs provider may use `docs-list.mjs` read-only ranking to discover repo-relative Markdown candidates by objective. When the active Pi system prompt already contains a selected AGENTS/doc file, the packet emits metadata instead of duplicating the whole content.
