...
- before choosing more provider work, review whether accumulated evaluated receipts show ranking failures, provider availability failures, accounting/omission mismatches, path-basis mismatches, live reload/registration gaps, or simply correct omissions; use aggregate summaries as triage only, not as canonical evidence or a substitute for owner-surface promotion; verify the live session has reloaded the latest schema before treating a missing tool as product behavior; wired provider outages should not be treated as planned-unwired adapter gaps;
