...
- maturity: `internal dogfood / package-cwd workspace-basis, repo-bounded AGENTS/CLAUDE instruction projection with Pi-style in-repo candidate priority, strict docs-list provider contract with deterministic executable trust, route-invariant provider telemetry, receipt-projection accounting, compact context_plan/context_pack detail membranes, activity/runtime-coverage-gated dogfood calibration with stored-evaluation status recomputation and centralized omission-follow-up class semantics, and JS object-boundary-hardened MVP with bounded-SCI sandboxing, deterministic SCI executable trust, safety/accounting receipts, local receipt evaluation, and validation-observable non-persistent aggregate summaries`
