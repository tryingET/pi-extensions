...
- read-only SCI provider seam that refuses workflow execution unless SCI read-only safety is explicitly confirmed, blocks existing source `.ontology` state, copies safe code path seeds through a temporary sandbox before SCI execution, and limits symbol searches to that bounded sandbox;
...
The main remaining product gap is not another provider adapter; it is still a growing body of live usefulness proof under the stricter pre-provider safety and accounting membrane. The latest iteration clarified that packet truth must come from one selected/omitted accounting path: provider-local budgets are cumulative, docs-list discovery remains available after unrelated unsafe seed omission, and session visibility is receipt truth only when the section actually survives budget selection. The dogfood export boundary is clearer: packet Markdown/details may show source-owned packet content for immediate use, but pasteable observation templates are receipt scaffolds only and must omit raw packet content, selected item paths, raw session/context-usage objects, and raw omission details. A local evaluator now helps compare filled redacted observations against packet predictions, and a local summarizer can aggregate multiple redacted observations/evaluations into packet-local status/provider/follow-up signals; both remain non-persistent and do not turn dogfood notes into AK evidence, FCOS closeout, session memory, provider authority, or task completion proof. The summarizer's own boundary is now sharper: malformed top-level inputs fail closed, combined item caps are enforced before JSON parsing, prototype-shaped labels remain countable, stored evaluation truncation is preserved, and invalid receipts block tuning claims rather than being silently ignored. Owner-extension registration has package-local smoke coverage, but live Pi sessions still need reload/activation before new tool schemas are available. The package now refuses artifact-prone SCI workflows unless read-only safety is explicitly confirmed, ignores artifact-creation bypass flags as authorization, fails closed when source `.ontology` state is observed under `cwd` or `repoRoot` (including symlinks), and executes SCI against a temporary sandbox copied from safe realpath-checked code path seeds rather than the source workspace; broad symbol-only SCI searches remain honest omissions until a wider verified sandbox/indexing strategy exists. First dogfood receipts show useful packets, no-packet outcomes, and post-reload template checks in [Dogfood measurement receipts — 2026-05-22](2026-05-22-dogfood-measurement-receipts.md), but the next frontier remains repeated evaluated receipts across implementation, review, and validation tasks that distinguish useful omissions from missing capability before ranking changes or new owner adapters are justified.
