...
- maturity: `internal dogfood / package-cwd workspace-basis and provider-route scoping hardened MVP with bounded-SCI sandboxing, safety/accounting receipts, local receipt evaluation, and non-persistent aggregate summaries`
- current strategic line: keep `context_plan` cheap and always available, keep `context_pack` activatable, preserve Markdown-primary output, dogfood package-cwd, monorepo-root, and provider-scoped seed behavior after reload, and use redacted live dogfood receipts plus packet-local evaluations/summaries to prove ranking/utility before adding AK/FCOS/Prompt Vault adapters or broadening SCI indexing
...
- before choosing more provider work, review whether accumulated evaluated receipts show ranking failures, provider availability failures, accounting/omission mismatches, path-basis mismatches, provider-route/query-seed mismatches, live reload/registration gaps, or simply correct omissions; use aggregate summaries as triage only, not as canonical evidence or a substitute for owner-surface promotion; verify the live session has reloaded the latest schema before treating a missing tool as product behavior; wired provider outages should not be treated as planned-unwired adapter gaps;
