...
- continue dogfooding `context_plan` / `context_pack` against real implementation, review, and validation tasks from both package cwd and monorepo-root contexts, recording whether packets reduced raw `read` / search / status churn under the stricter safety/accounting membrane, treating the redacted observation template as the pasteable receipt, `context_dogfood_evaluate` as single-receipt calibration, and `context_dogfood_summarize` as non-persistent multi-receipt calibration while keeping full packet content local to the active turn;
