...
export async function checkAkTaskExecutionBinding(
  ...
  if (taskResult.code !== 0) return `AK task #${taskId} could not be read`;
  ...
  if (readyResult.code !== 0) return `AK task #${taskId} readiness could not be verified`;
  try {
    const ready = JSON.parse(readyResult.stdout ?? "") as unknown;
    if (!Array.isArray(ready) || !ready.some((item) => item?.id === taskId)) {
