...
| `visible-loop` | Launch a clean visible Ghostty Pi tab for each iteration after exactly one execution binding is supplied with `--task`, `--objective`, or `--candidate`; queue the bounded product membrane, implementation, review/fixup, posture, and commit sequence; terminate truthfully as deferred/blocked when no lawful slice remains; `--delegate-commit` resolves `/commit` and delegates it to `dispatch_subagent` |
...
When a later binding check finds that the same slice is owner-gated or otherwise unlawful, the child uses the internal `visible_loop_child_defer` terminal tool instead of settling with prose. The tool records a mode-0600 no-replace terminal record, reports `deferred` or `blocked` with bounded owner/task/decision/trigger references and next actions, cancels every remaining prompt, and never emits iteration/loop completion. Status JSONL, tool output, UI, and intercom surface that local disposition but remain non-authoritative. The old config cannot be resumed: resolve or review each reference at its owning surface, then launch a fresh loop with a newly valid binding. For AK work, that means the deferral must be resolved and the task must pass the ordinary ready/claim admission check before `/visible-loop --task AK-ID ...` can launch.
