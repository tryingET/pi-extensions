...
| `nexus-loop` | Launch the same bound machinery for an existing implementation slice: `/deep-review`, consolidated Nexus/atomic fixup, product-posture refresh, and resolved `/commit` delegation; it cannot launch unbound to select product direction |
...
4. verify `/nexus-loop --objective "harden the named existing implementation" --count 2` uses the same binding/iteration/intercom behavior while queueing only four real prompts: `/deep-review`, consolidated Nexus/atomic fixup with Prompt Vault grounding, posture refresh, and resolved `/commit` delegation
