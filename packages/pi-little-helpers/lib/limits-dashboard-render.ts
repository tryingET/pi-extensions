...
import { resetGuidance } from "./reset-capabilities.ts";
...
export function renderAccountDetails(
  ...
  if (!snapshot) {
    ...
    return [...lines, ...resetGuidance(row.account.provider)];
  }
  ...
  lines.push(
    ...
    theme.fg(
      "dim",
      "Select this subscription, then /resets status or /resets use (confirmation required).",
    ),
  );
  return lines;
}
