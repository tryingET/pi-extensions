...
export class LimitsDashboard implements Component, Focusable {
  ...
  private guide(width: number, height: number): string[] {
    const help = [
      ...
      "Reset inventory: Codex locally; Grok and ZCode through compatible sub-core.",
      "Native redemption remains Codex-only. ZCode inventory needs explicit app sign-in tokens.",
      "After closing: /resets status inspects the active subscription; /resets use requires confirmation.",
      ...
      "↺0 = known none; ? = count not read; login = sign-in required; n/s = unsupported here.",
      "multi = separate 5h/weekly reset inventories; inspect details, never sum card counts.",
      "Expiry ? = missing dates. Unsupported / unknown never means zero resets.",
      ...
      "Before name: ↻ checking, … queued, ~ partial, ! failed, old = failed check with retained data.",
