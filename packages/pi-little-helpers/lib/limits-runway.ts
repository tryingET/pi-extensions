...
import { resetInventoryLines } from "./reset-inventory.ts";
...
export function bankedResetFacts(row: LimitsAccountRow, now: number) {
  const windows = row.snapshot?.resetInventory?.windows;
  if (windows?.length) {
    const count =
      windows.length === 1
        ? windows[0].availableCount
        : windows.every((w) => w.availableCount === 0)
          ? 0
          : undefined;
    const expiry = nextCreditExpiry(row);
    return {
      count,
      label: count === undefined ? "multi" : `↺${count}`,
      expiry,
      partialDates: false,
      tone: creditExpiryTone(expiry, now),
    };
  }
  if (resetCapability(row.account?.provider ?? row.snapshot?.provider ?? "").inventory !== "native")
    return undefined;
...
export function bankedResetSummary(row: LimitsAccountRow, now: number) {
  ...
  if (facts.count === undefined)
    return {
      text: `${facts.label} ${facts.expiry === undefined ? "expiry?" : expiryText(facts, now)}`,
      tone: facts.tone,
    };
...
function healthMarker(row: LimitsAccountRow): string {
  if (row.error || row.status === "error")
    return row.snapshot?.usage || row.snapshot?.credits || row.snapshot?.money ? "old " : "! ";
...
export function overviewCells(
  ...
  const facts = bankedResetFacts(row, now);
  const capability = resetCapability(row.account.provider);
  const unknownBank = capability.inventory === "sub-core";
  const needsLogin = ["AUTH_REQUIRED", "NO_CREDENTIALS"].includes(
    row.snapshot?.resetInventory?.code ?? "",
  );
  ...
  return [
    ...
    theme.fg("dim", facts ? facts.label : unknownBank ? (needsLogin ? "login" : "?") : "n/s"),
    theme.fg(
      facts?.tone ?? "dim",
      facts ? expiryText(facts, now) : unknownBank ? "unknown" : "n/s",
    ),
  ];
}
...
export function providerDetailLines(snapshot: LimitsSnapshot, theme: Theme, now: number): string[] {
  const lines: string[] = snapshot.resetInventory
    ? ["BANKED RESETS · read-only inventory", ...resetInventoryLines(snapshot.resetInventory), ""]
    : [...resetGuidance(snapshot.provider), ""];
...
export function renderTimeline(rows: LimitsAccountRow[], theme: Theme, now: number): string[] {
  const events = rows
    .flatMap((row) => {
      ...
      const scopedCredits = (row.snapshot?.resetInventory?.windows ?? []).flatMap((w) =>
        w.resets.map((card) => ({
          at: Date.parse(card.expiresAt ?? ""),
          label: `${row.account.label} · ${w.scope === "five_hour" ? "5h" : "Week"}`,
          kind: "credit expires",
        })),
      );
      return [...windows, ...credits, ...scopedCredits].filter((event) =>
