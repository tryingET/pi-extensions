...
const refusalCases = [
  ...
  ].map((surfaceId) => ({ name: `zero/no-target representation ${surfaceId}`, surfaceId })),
  { name: "missing controller", controllerGhostty: undefined },
  ...[0, -1, 1.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1, "111"].map((pid) => ({
    name: `invalid PID ${pid}`,
    controllerGhostty: { pid, exe: ORIGIN_EXE },
  })),
  {
    name: "unreadable controller is not a stub",
    read: (pid) => (pid === 222 ? ORIGIN_EXE : undefined),
  },
  {
    name: "mismatched controller build",
    read: (pid) => (pid === 111 ? "/usr/bin/ghostty" : ORIGIN_EXE),
  },
  {
    name: "throwing identity read",
    read() {
  ...
  ...[undefined, "", "-1", "18446744073709551616", " 1", "+1", "wat"].map((surfaceId) => ({
    name: `invalid surface ${surfaceId}`,
    surfaceId,
  })),
  ...
  {
    name: "duplicate well-known row",
    rows: `${daemonRows}com.mitchellh.ghostty 222 ghostty user :1.43 unit - -\n`,
  },
  {
    name: "ambiguous originator",
    rows: `${originRow}:1.98 111 ghostty user :1.98 unit - -\n${daemonRows}`,
  },
  ...
  {
    name: "owner PID disagrees with connection",
    rows: daemonRows.replace("com.mitchellh.ghostty 222", "com.mitchellh.ghostty 333"),
  },
  {
    name: "unique connection differs",
    rows: originRow.replace("user :1.99", "user :1.98") + daemonRows,
  },
  { name: "missing daemon unique connection", rows: daemonRows.split("\n").slice(1).join("\n") },
  {
    name: "nameless controller cannot own daemon",
    rows: "com.mitchellh.ghostty 111 ghostty user :1.43 unit - -\n",
  },
  ...
  {
    name: "no cross-family fallback",
    rows: daemonRows.replace("com.mitchellh.ghostty", "com.tryinget.ghosttysidequest"),
  },
...
for (const item of refusalCases) {
  test(`resolver refuses ${item.name} without dispatch`, async () => {
    const calls = [];
    const target = await resolveControllerGhosttyDbusTarget({
      controllerGhostty: Object.hasOwn(item, "controllerGhostty")
        ? item.controllerGhostty
        : { pid: 111, exe: ORIGIN_EXE },
...
test("resolver refuses a selected owner disappearing during executable recheck", async () => {
  ...
  const target = await resolveControllerGhosttyDbusTarget({
    ...
    surfaceId: SURFACE,
    readProcessExecutable(pid) {
      return pid === 111 || (pid === 222 && ++daemonReads === 1) ? ORIGIN_EXE : undefined;
    },
...
for (const scenario of [
  ...
  test(
    `enabled stagger: ${scenario} releases a queued successor`,
    {
    ...
    async () => {
      ...
      const describeGate = new Promise((resolve) => {
        releaseDescribe = resolve;
      });
      const described = new Promise((resolve) => {
        announceDescribe = resolve;
      });
      const successorIdentity = new Promise((resolve) => {
        announceSuccessorIdentity = resolve;
      });
      ...
      const spawn = (attempt) => {
        ...
        const exec = (command, args) => {
          ...
          if (command === "busctl" && args.includes("Activate")) {
            ...
            if (first && scenario === "activation throw")
              throw new Error("synthetic activation throw");
        ...
        const { tools } = registerExtension(
          createSidequestExtension({
            ...
            env: {
              ...
              PI_SIDEQUEST_LAUNCH_STAGGER_MS: String(staggerMs),
              XDG_STATE_HOME: cwd,
              XDG_RUNTIME_DIR: cwd,
            },
        ...
        return tool.execute(
          `AK5638-${attempt}`,
          {
            objective: `AK5638 ${attempt} objective`,
            cwd,
            reportBack: "none",
          },
          undefined,
      ...
      try {
        ...
        assert.deepEqual(
          events,
          ["first:Describe"],
          "queued successor cannot inspect while first holds the slot",
        );
        assert.equal(
          starts.length,
          0,
          "neither session transport is admitted during held inspection",
        );
        releaseDescribe();

        // Must finish while first transport is still deliberately unresolved in pending mode.
        const secondResult = await wait(
          successor,
          "successor completion before releasing first transport",
        );
        ...
        if (scenario === "pending transport") {
          assert.equal(
            firstSettled,
            false,
            "stagger serializes transport starts, not transport settlement",
          );
          assert.deepEqual(
            starts.map(({ attempt }) => attempt),
            ["first", "successor"],
          );
          assert.ok(
            events.indexOf("successor:transport-start") > events.indexOf("first:transport-start"),
          );
        ...
        if (scenario === "Describe refusal" || scenario === "identity drift") {
          ...
          assert.match(
            ...
            scenario === "Describe refusal"
              ...
              : /identity changed during capability inspection/,
          );
          assert.deepEqual(
            starts.map(({ attempt }) => attempt),
            ["successor"],
          ...
          assert.ok(
            starts[1].at - starts[0].at >= staggerMs,
          ...
          assert.equal(
            firstResult.details.effectDisposition,
          ...
          if (scenario === "activation throw") {
            ...
            assert.ok(
              events.indexOf("first:settled") < events.indexOf("successor:transport-start"),
              "throwing first transport settles without wedging the spaced successor",
            );
          } else
            assert.ok(
              events.indexOf("successor:settled") < events.indexOf("first:settled"),
              "successor both dispatches and settles before first transport settles",
            );
        }
        assert.deepEqual(
          events.filter((event) => event.endsWith(":Describe")),
          ["first:Describe", "successor:Describe"],
        );
        assert.ok(
          events.indexOf("successor:Describe") > events.indexOf("first:inspection-released"),
        );
        ...
        assert.ok(
          calls.every(
            ({ command, args }) =>
              ...
              (command === "busctl" &&
                (args[1] === "list" || args.includes("Describe") || args.includes("Activate"))),
          ),
        ...
        assert.ok(
          inspections.every(
            ({ args }) =>
              ...
              !args.some(
                (arg) =>
                  arg.includes("AK5638 first objective") ||
                  arg.includes("AK5638 successor objective"),
              ),
          ),
