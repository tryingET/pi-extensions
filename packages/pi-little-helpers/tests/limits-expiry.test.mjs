...
test("key and wallet remain independently labelled at every table width; no quota renewal is invented", () => {
  ...
  for (const width of [15, 18, 47]) {
    ...
    assert.deepEqual(cells.slice(2), ["—", "n/s", "n/s"]);
  }
...
test("zero, missing and partially undated credits remain distinct; unverified providers have no native bank facts", () => {
