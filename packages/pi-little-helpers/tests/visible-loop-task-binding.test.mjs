...
test("AK task execution binding admits only current-repo ready or live-claimed work", async () => {
  assert.equal(
    await checkAkTaskExecutionBinding(
      execFrom({ id: 42, repo: "/repo", status: "pending", active_deferral: null }, [{ id: 42 }]),
      "/repo/package",
      42,
    ),
    undefined,
...
test("AK task execution binding rejects stale, deferred, cross-repo, and malformed work", async () => {
  const cases = [
    ...
    {
      name: "cross repo",
      task: { id: 42, repo: "/other", status: "pending" },
      ready: [{ id: 42 }],
    ...
    {
      name: "done",
      task: { id: 42, repo: "/repo", status: "done" },
      ready: [],
      expected: /not pending or claimed/,
    },
    {
      name: "missing lease",
      task: { id: 42, repo: "/repo", status: "claimed" },
      ready: [],
    ...
    {
      name: "not ready",
      task: { id: 42, repo: "/repo", status: "pending" },
      ready: [],
