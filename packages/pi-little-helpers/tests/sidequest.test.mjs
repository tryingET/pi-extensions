...
import {
  assertNoSessionDispatch,
  assertTargetRefusalBookkeeping,
  targetBusRows,
  targetLaunchCase,
} from "./sidequest-target-refusal-harness.mjs";
...
test("nonzero exact activation stays indeterminate and never retries a window", async () => {
  const outcome = await targetLaunchCase({
    activation: { code: 1, stderr: "activation may have dispatched" },
  });
...
test("targeted launch orders identity, read-only capability, identity recheck, then one activation", async () => {
  ...
  assert.deepEqual(
    ...
    ["help", "identity", "Describe", "identity", "Activate"],
  );
  ...
  assert.ok(
    outcome.calls.slice(0, -1).every(({ args }) => !args.includes("PRIVATE_SESSION_PAYLOAD")),
  );
});
...
test("sidequest rejects a killed D-Bus activation even when the executor reports code zero", async () => {
  ...
  assert.ok(
    execStub.calls.some(({ command, args }) => command === "busctl" && args.includes("Activate")),
  );
  assert.ok(
    !execStub.calls.some(
      ({ command, args }) =>
...
test("unsupported exact action refuses rather than omitting surface targeting", async () => {
  assertNoSessionDispatch(
    await targetLaunchCase({ description: { code: 0, stdout: '(bgav) true "as" 0' } }),
  );
});
...
test("fresh-handoff launches a clean Pi session and auto-submits exactly one generated prompt", async () => {
  ...
  const execStub = createExecStub(({ command, args }) => {
    ...
    if (
      args[0]?.startsWith("--working-directory=") &&
      args[1]?.startsWith("--surface-id=zz-invalid-surface-id")
    ) {
...
for (const item of [
  ...
  {
    name: "killed Describe",
    description: { code: 0, killed: true, stdout: '(bgav) true "(tas)" 0' },
  },
  { name: "throwing Describe", describeThrows: true },
  { name: "owner disappears after Describe", freshRows: "" },
  {
    name: "owner connection changes after Describe",
    freshRows: targetBusRows.replaceAll(":1.11", ":1.99"),
  },
]) {
...
test(
  "target refusal keeps admission/worktree bookkeeping and records ordinary launch failure",
  assertTargetRefusalBookkeeping,
);
