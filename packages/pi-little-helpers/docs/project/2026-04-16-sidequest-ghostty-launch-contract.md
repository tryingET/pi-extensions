...
14. Controller-process D-Bus targeting must resolve a single unique bus name whose reported PID equals the observed Ghostty ancestor PID, encode the surface id as a bounded unsigned 64-bit integer, and pass launch arguments as argv elements rather than shell-concatenating a `busctl` command. If direct activation fails, use the existing honest new-window fallback.
