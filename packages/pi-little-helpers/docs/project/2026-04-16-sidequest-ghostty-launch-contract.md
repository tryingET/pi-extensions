...
If the active Ghostty session cannot prove that, the launcher must open a **new Ghostty window** and say why. `/handoff-tab` additionally launches a clean Pi session: it never passes `--fork`, and it auto-submits exactly one prompt generated by the `pi-session-compaction` owner surface.
