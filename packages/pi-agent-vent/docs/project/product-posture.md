...
- current capability baseline: local append-only vent capture with optional local tool/package facets, recurrence grouping, local facet summary, local operator review queue with read-only category/tag/tool/package facet filters, state-aware local next-action guidance, bounded representative-sample detail, review-state events, append-only recurrence curation projections with remove/undo events, diagnostic-state load membrane with privacy metadata recomputation, facet-aware draft-only owner-surface text generation, lifecycle stats/export projections, lock/hash-guarded confirmation-gated retention archive/restore with duplicate-id-safe record selection, local backup receipts, and rollback safeguards, advisory candidate-incident heuristic, redaction/minimization, `/agent_vent` inspection command, toolbox discovery, ASC/self companion routing
...
The landed baseline lists groups by recurrence priority, supports read-only category/tag/tool/package facet filters, shows state-aware local next-action guidance, shows representative summaries/sample ids through recurrence projections, offers bounded read-only representative-sample expansion for a single group, and records append-only local review-state events without mutating owner systems. Remaining product depth is operator ergonomics: richer routing-hint views and clearer review-flow affordances, not more authority.
...
The next highest-leverage slice should assume the facets/review/filter/detail/draft/curation/retention membrane is the baseline and should not broaden authority. Retention now has transaction-oriented safeguards (store/review/curation-hash tokens, append/archive locking, duplicate-id-safe selected-record removal, stale-lock cleanup, receipt-failure rollback, derived restore tokens, realpath backup containment, quoted rollback commands, and stale-restore checks). Local facet labels and filters are diagnostic hints only; Prompt Vault, AK, GitHub, incident, evidence, ROCS, ASC/self, and toolbox surfaces remain separate owners. The next slice should likely improve routing-hint/review-flow affordances without treating local labels as canonical routing, and should keep new agent-local artifacts outside package validation scans. Do not spend the next slice on hard-delete unless a new decision explicitly supersedes [ADR 2026-05-22](../adr/2026-05-22-agent-vent-retention-delete-policy.md).
