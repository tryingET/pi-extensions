...
`/agent-vent` remains a compatibility alias for users who prefer kebab-case slash commands. Review queue and detail output include advisory human-review hints, exact local next-action commands for review state, draft-only handoff targets, and retention preview eligibility; these are guidance only and do not route, file, create, declare, assign, record evidence, publish, or mutate owner systems.
