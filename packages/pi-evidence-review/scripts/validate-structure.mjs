...
function validatePackageJson() {
  ...
  if (p.pi?.prompts !== undefined) {
    fail("package.json pi.prompts must be absent; this package activates only evidence-review");
  }

  const requiredPeers = ["@earendil-works/pi-coding-agent", "@earendil-works/pi-tui"];
  ...
  if (!Array.isArray(p.files) || p.files.length < 1) {
    ...
    if (p.files.includes("prompts")) {
      fail("package.json files must not include generic prompts");
    }
