...
## Package-specific rules (pi-agent-registry)

- Implements the agent manifest convention v1 (monorepo
  `docs/project/2026-08-27-agent-manifest-convention.md`). Fleet-layout
  policy owner is `softwareco-agents/docs/agent-registry.md`; do not
  duplicate fleet policy here.
- Execution is ASC-owned: never add spawn/session/capacity machinery to this
  package; extend the ASC `extraSkillProfileResolver` seam instead.
- Tests may read the real fleet fixture
  (`~/ai-society/agents/agent-adoption-steward`) and the real
  engineering-core `skills/profiles.json`; treat their absence as a skip
  candidate only if the workspace is not this operator's machine.
- Live activation: `pi install` this package path, `/reload`, then verify
  with a real `agent_registry` call.
