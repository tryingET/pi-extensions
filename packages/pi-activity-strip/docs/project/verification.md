...
- Pi's `/activity-strip stop` and `/activity-strip-stop` go through the same `stopRuntime()` as the CLI, so they also return only after the exit; they used to report "Stopping" as soon as the broker accepted.
