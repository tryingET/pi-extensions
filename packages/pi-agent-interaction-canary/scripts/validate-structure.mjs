...
function validatePackageJson() {
  ...
  const requiredPeers = ["@earendil-works/pi-coding-agent"];
  ...
  if (p.private !== true) {
    fail("package.json private must be true for the experimental canary");
  }

  if (p.publishConfig !== undefined) {
    fail("package.json publishConfig must be absent for the private canary");
  }

  if (
    p.scripts?.["release:check"] !== undefined ||
    p.scripts?.["release:check:quick"] !== undefined
  ) {
    fail("package.json release scripts must be absent for the private canary");
  }
  ...
  if (!Array.isArray(p.files) || p.files.length < 1) {
    ...
    if (p.files.includes("prompts")) {
      fail("package.json files must not expose generated prompts from the private canary");
    }
