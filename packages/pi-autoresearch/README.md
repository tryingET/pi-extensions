...
- `autoresearch_runtime_status`
  ...
  - can build a non-mutating Oracle-ready evidence packet (`action: "oracle_evidence"`, packet kind `autoresearch.oracle_evidence.v1`) with run-level empirical records and a DSPx-owner preflight handoff; it does not write Oracle Postgres, migrate local `coordinates.db`, write AK/KES, choose winners, or promote
