...
- `autoresearch_runtime_status`
  ...
  - can build a non-mutating Oracle-ready evidence packet (`action: "oracle_evidence"`, packet kind `autoresearch.oracle_evidence.v1`) and locally export it for DSPx owner preflight (`action: "oracle_evidence_export"`, default `.autoresearch/oracle_evidence.json`) with run-level empirical records; it does not write Oracle Postgres, migrate local `coordinates.db`, write AK/KES, choose winners, or promote
