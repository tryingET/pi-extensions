...
- `autoresearch_runtime_status`
  ...
  - can build a candidate-result packet (`action: "candidate_result"`, packet kind `autoresearch.candidate_result.v1`) and locally export it for owner-visible candidate-wave review (`action: "candidate_result_export"`, default `.autoresearch/candidate-result.json`); export is an explicit local write unavailable in read profile, constrained under `cwd/.autoresearch/`, and overwrite-gated, and it does not mutate candidate lifecycle, worktrees, AK/KES/evidence, or promotion state
