...
- bounded repo/problem autoplan that proposes campaign config, benchmark/check commands, scope, risks, exact setup call, optional metric-emitting `autoresearch.sh` scripts for safe generic benchmark mismatches, measurement-contract gating for whether a proposed metric may drive baseline optimization, duplicate benchmark/check command detection for package-script aliases, noise-aware duration-metric interpretation, optional DSPx `program-gen` handoff intent, and evidence-only DSPx behavior advisory proposals for an `AutoresearchSetupPlanner` candidate
