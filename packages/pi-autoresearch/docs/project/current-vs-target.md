...
- bounded repo/problem autoplan that proposes campaign config, benchmark/check commands, scope, risks, exact setup call, optional DSPx `program-gen` handoff intent, and evidence-only DSPx behavior advisory proposals for an `AutoresearchSetupPlanner` candidate
