...
- current strategic line: measurement trust, operator clarity, export-before-review packet inventory UX, threshold-style success semantics, confirmation UX, and consumer-driven external proof before broader autonomy
- release posture: package checks pass and the canonical dogfood loop has now been exercised against the package itself; the Level-4 dogfood loop has also measured candidate UX changes through `autoresearch_candidate_bind -> autoresearch_runtime_run -> candidate_result_export -> review_candidate_wave`; product posture remains pre-public until metric-readiness UX, confirmation affordances, and at least one non-AK consumer adapter proof are clearer
...
2. keep the measured packet inventory route obvious: `/autoresearch export` first for candidate-result packet completeness and `export_visibility_blockers`, `/autoresearch review` only for final owner lifecycle decisions;
3. operator posture clarity, especially when success is threshold-satisfied rather than metric-improving;
4. metric readiness, threshold-style success semantics, and baseline-drift protection;
5. keep the canonical dogfood playbook and workflow-contract benchmark current with actual tool schemas;
6. keep orchestrator-side live/manifest/self-hosting supervision proofs aligned with real package artifacts and exact-task evidence boundaries;
7. one external adapter proof only after a real consumer needs it.
...
The dashboard summarizes current posture, metric contract, metric-readiness/trust posture, confidence/noise interpretation, candidate decision summary, candidate lifecycle policy, a unified Authority handoff section (closeout, `ak_evidence`/suggested `evidence_record(...)`, `learning_export` -> owner-routed KES adapter, and `oracle_evidence_export` -> DSPx owner preflight), and next legal surfaces without running benchmarks, mutating worktrees, writing AK/KES/Oracle, or promoting.
...
A browser export slice is now landed: `/autoresearch export` writes `.autoresearch/autoresearch-dashboard.html`, opens it in the browser when possible, and refreshes the file every ~2s for the current Pi session. It is explicitly the measured packet inventory inspection surface before owner review: candidate-result packets, selected lanes, exported counts, and `export_visibility_blockers` are visible there before `/autoresearch review` is used for final owner decisions. It includes a metric-readiness/trust card so long-running campaigns can be watched for threshold readiness, duration under-sampling, and baseline-drift blockers without scraping candidate-decision text. It also renders the same unified Authority handoff section as the text dashboard so closeout, AK evidence, learning/KES, and Oracle/DSPx preflight next calls remain visible while the export itself writes only the dashboard HTML. When matrix artifacts are present, the export switches into matrix-campaign mode: matrix cell progress, selected lanes, packet inventory, visibility blockers, and next legal actions become the primary progress cards, while empty local single-segment runtime fields are labeled as an auxiliary snapshot. The graph also switches sources in matrix mode: it prefers matrix closeout/candidate-result metric points and falls back to cell-completion progress instead of showing an empty runtime-only chart when no local `autoresearch.jsonl` series exists. `/autoresearch export off` stops that session-local refresher. The export is read-only and does not own execution or promotion.
