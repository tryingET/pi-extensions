...
export function buildAutoresearchHelpText(status: AutoresearchRuntimeStatus): string {
  ...
  return [
    "# /autoresearch",
    "",
    "The bounded runtime kernel is available for local benchmark/check execution, machine projection, append-only receipt/event logging, governed Prompt Vault decision requests, bounded finalization orchestration, and manifest-driven llama.cpp campaign planning/fork preparation/stage binding plus package-local campaign receipt/status projection and exact-task AK-binding snapshot derivation.",
    "This package now owns bounded finalization planning, approval, local branch materialization, checked manifest-driven branch/lane planning, one exact 41/42/43 stage-binding surface, one projection-only llama.cpp campaign status artifact, and one non-mutating AK-ready manifest-campaign binding helper for brownfield workflows; it still does not own the autonomous loop, direct AK mutation policy, or remote review choreography.",
    "",
    "## Available surfaces",
    `- command: /${status.commandName}`,
    `- tools: ${status.toolNames.join(", ")}`,
    ...
    `- use ${AUTORESEARCH_LLAMACPP_CAMPAIGN_TOOL_NAME} to load a typed llama.cpp benchmark campaign manifest, emit the exact 41/42/43 branch-lane matrix, plan/apply fork preparation, plan/apply one exact stage binding, or derive one exact AK-ready binding snapshot`,
    "",
    ...configurationBlock,
    "",
    "## Local artifact plan",
